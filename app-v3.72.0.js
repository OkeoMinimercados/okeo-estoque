...
function friendlyAuthError(e){
  ...
  if(code==="AUTH_INVALID")return "Usuário ou senha inválidos. No celular, confira se o preenchimento automático não inseriu uma senha antiga.";
  ...
  try{
    ...
    if(e?.message==="TIMEOUT_BASE_CENTRAL"){
      ...
      m.textContent="Falha no login: "+friendlyAuthError(e)
        +(sessionStorage.getItem("okeo_last_auth_reason")?` [${sessionStorage.getItem("okeo_last_auth_reason")}]`:"")
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=3.72.0",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="3.72.0"){
        localStorage.setItem("okeo_frontend_cache_version","3.72.0")
      }
    })
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.72.json",{cache:"no-store"});
