...
async function cdInboundRows(){const purchases=await all("purchases"),rows=[];for(const p of purchases.filter(x=>x.source==="WEEKLY"))for(const it of(p.items||[]))if(it.route==="CD"&&it.unitId&&!["SUPPLIED","CUT"].includes(it.status))rows.push({...it,purchaseId:p.id,purchaseAt:p.at||p.date||""});return rows.sort((a,b)=>String(b.purchaseAt).localeCompare(String(a.purchaseAt)))}

async function renderCdInboundPurchases(){
  const box=$("#cdInboundList");if(!box)return;
  const rows=await cdInboundRows();
  const supplierCurrent=$("#cdInboundSupplier")?.value||"",unitCurrent=$("#cdInboundUnit")?.value||"",status=$("#cdInboundStatus")?.value||"",q=normalizeText($("#cdInboundSearch")?.value||"");
  const suppliers=[...new Set(rows.map(x=>x.supplier).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  const units=[...new Map(rows.map(x=>[x.unitId,x.unit])).entries()].sort((a,b)=>String(a[1]).localeCompare(String(b[1])));
  const ss=$("#cdInboundSupplier"),us=$("#cdInboundUnit");
  if(ss){ss.innerHTML='<option value="">Todos</option>'+suppliers.map(n=>`<option value="${esc(n)}">${esc(n)}</option>`).join("");if(suppliers.includes(supplierCurrent))ss.value=supplierCurrent}
  if(us){us.innerHTML='<option value="">Todos</option>'+units.map(([id,n])=>`<option value="${esc(id)}">${esc(n)}</option>`).join("");if(units.some(([id])=>id===unitCurrent))us.value=unitCurrent}
  const supplier=$("#cdInboundSupplier")?.value||"",unit=$("#cdInboundUnit")?.value||"";
  const filtered=rows.filter(x=>(!supplier||x.supplier===supplier)&&(!unit||x.unitId===unit)&&(!status||x.status===status)&&(!q||normalizeText([x.product,x.ean,x.supplier,x.unit,x.purchaseId].join(" ")).includes(q)));
  if($("#cdInboundSummary"))$("#cdInboundSummary").textContent=`${filtered.length} item(ns) • ${qtyFmt(filtered.reduce((s,x)=>s+(+x.qty||0),0))} unidade(s)`;
  box.innerHTML=filtered.length?`<div class="dtable"><table><thead><tr><th>Compra</th><th>Fornecedor</th><th>Condomínio</th><th>Produto</th><th>Pedida</th><th>Recebida</th><th>Status</th><th>Validades / lotes</th><th>Ação</th></tr></thead><tbody>${filtered.map(x=>`<tr><td>${esc(x.purchaseId)}</td><td>${esc(x.supplier||"—")}</td><td>${esc(x.unit)}</td><td><b>${esc(x.product)}</b><br><small>${esc(x.ean)}</small></td><td>${qtyFmt(x.qty)}</td><td>${x.receivedQty===null||x.receivedQty===undefined?"—":qtyFmt(x.receivedQty)}</td><td><b>${purchaseFlowStatusLabel(x)}</b></td><td>${(x.expiryLots||[]).map(l=>`${qtyFmt(l.qty)} • ${esc(l.expiry)}${l.lot?` • ${esc(l.lot)}`:""}`).join("<br>")||"—"}</td><td><button type="button" onclick="openPurchaseFlowForItem('${x.unitId}','${jsQuote(x.supplier||"")}','${x.ean}')">Conferir</button></td></tr>`).join("")}</tbody></table></div>`:'<p class="muted">Nenhuma mercadoria em Pré-Separado em Compras com os filtros selecionados.</p>'
}
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.51.json",{cache:"no-store"});
...
async function openSupplierProducts(idv){setRegistryTab("suppliers");await hydrateSupplierCatalog();await refreshSupplierRegistryFilter(true);const sel=$("#supplierRegistryFilter");if(!sel)return;sel.value=idv;await loadSupplierProductSelection();const drop=$("#supplierProductDropdown");drop?.classList.remove("hidden");await renderSupplierProductList();setTimeout(()=>drop?.scrollIntoView({behavior:"smooth",block:"start"}),50)}
