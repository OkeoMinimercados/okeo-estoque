...
function bind(){
  ...
  if($("#cunit"))$("#cunit").addEventListener("change",()=>{if(controlStockMode==="point")loadPhysicalCountProducts(false).catch(console.warn)});
}
...
async function getPhysicalCountUnitProducts(unitId){
  const [products,stock]=await Promise.all([all("products"),all("stock")]),pm=new Map(products.map(p=>[p.ean,p]));
  // Produtos já registrados na unidade, inclusive saldo zero se existe registro de estoque.
  return stock.filter(s=>s.unitId===unitId).map(s=>({ean:s.ean,product:pm.get(s.ean)?.name||s.product||s.ean,p:pm.get(s.ean)||null,predicted:Math.round(+s.qty||0),observed:null,avgCost:+s.avgCost||+s.lastCost||0}))
...
function renderPhysicalCountProductList(){
  ...
  box.innerHTML=rows.length?`<div class="dtable"><table><thead><tr><th>Produto</th><th>EAN</th><th>Previsto</th><th>Observado</th><th>Diferença</th></tr></thead><tbody>${rows.map(r=>`<tr id="pcrow_${r.ean}">
    <td><b>${esc(r.product)}</b></td><td>${r.ean}</td><td>${qtyFmt(r.predicted)}</td>
  ...
  </tr>`).join("")}</tbody></table></div>`:'<p class="muted">Nenhum produto encontrado para esta unidade.</p>'
}
...
async function renderControlPoint(){
  ...
  if(activeControlId&&controlStockMode==="point")await loadPhysicalCountProducts(false);
}
...
async function backup(){const o={version:"3.20.0",createdAt:new Date().toISOString(),stores:{}};for(const s of SYNC_STORES)o.stores[s]=await all(s);const b=new Blob([JSON.stringify(o,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="OKEO_CORE_Backup_V3_18.json";a.click()}