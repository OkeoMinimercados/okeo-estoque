...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=3.60.0",{updateViaCache:"none"});
    await reg.update();
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.60.json",{cache:"no-store"});
...
async function supplierVisibleProducts(){
  ...
  const scoped=rows.filter(p=>{
    const linked=p.supplierId===sid||normalizeText(p.supplier||"")===normalizeText(supplier.name);
    const unassigned=!p.supplierId&&!String(p.supplier||"").trim();
    return mode==="UNASSIGNED"?unassigned:linked
  });
  return scoped.filter(p=>!q||normalizeText([p.name,p.ean,p.segment,p.vmPayName,...(p.aliases||[])].filter(Boolean).join(" ")).includes(q)).sort((a,b)=>String(a.name).localeCompare(String(b.name)))
}
