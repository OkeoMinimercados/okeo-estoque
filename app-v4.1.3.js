...
const EXPECTED_BACKEND_VERSION="4.1.3";
...
async function setBackendUrl(url){
  ...
  try{await put("settings",{id:"backend",url:clean,customAllowed:!isOfficial})}catch(e){}
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=4.1.3",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="4.1.3"){
        localStorage.setItem("okeo_frontend_cache_version","4.1.3")
      }
    })
...
async function verifyBackendVersion(expected="4.1.3"){
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("4.1.3");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 4.1.3.`;
      alert(`Publique primeiro o backend 4.1.3. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    ...
    const resumable=progress&&progress.version==="4.1.3";
    if(!resumable){
      ...
      await backup();
      progress={version:"4.1.3",storeIndex:0,rowIndex:0,sent:0,startedAt:new Date().toISOString()};
...
async function runDeepHealthCheck(){
  ...
  if(remote.ok){
    ...
    add("Backend",status.version==="4.1.3",`publicado v${status.version||"?"} • esperado v4.1.3`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="4.1.3",`Versão ${status.version||"?"}`);
