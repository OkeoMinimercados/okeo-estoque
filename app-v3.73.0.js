...
async function createFirstAdmin(){
  ...
  try{
    const r=await authPost("bootstrap_admin",{username,password:p1},{url:DEFAULT_BACKEND_URL,timeoutMs:12000});
    if(!r.ok)throw new Error(r.error||"Falha ao criar administrador");
    $("#bootstrapBox")?.classList.add("hidden");
    $("#user").value=username;
    $("#pass").value="";
    $("#bootstrapPass").value="";
    $("#bootstrapPass2").value="";
    m.textContent="Administrador criado nesta Base Central. Digite a nova senha acima e entre no sistema.";
    await refreshLoginConnectionStatus(false)
  }catch(e){
    m.textContent="Falha ao criar Administrador: "+friendlyAuthError(e)
  }finally{
    if(btn){btn.disabled=false;btn.textContent="Criar primeiro Administrador"}
  }
}
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=3.73.0",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="3.73.0"){
        localStorage.setItem("okeo_frontend_cache_version","3.73.0")
      }
    })
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.73.json",{cache:"no-store"});
