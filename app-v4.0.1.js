...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=4.0.1",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="4.0.1"){
        localStorage.setItem("okeo_frontend_cache_version","4.0.1")
      }
    })
...
async function runDeepHealthCheck(){
  ...
  try{
    ...
    add("Backend",status.version==="4.0.1",`v${status.version||"?"}`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="4.0.1",`Versão ${status.version||"?"}`);
