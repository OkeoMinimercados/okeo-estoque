...
async function normalizeExclusiveSupplierAssignments(){
  const [products,suppliers,offers]=await Promise.all([all("products"),all("suppliers"),all("supplierOffers")]),sm=new Map(suppliers.map(x=>[normalizeText(x.name),x])),byEan=new Map();for(const o of offers.filter(x=>x.active!==false)){if(!byEan.has(o.ean))byEan.set(o.ean,[]);byEan.get(o.ean).push(o)}
  for(const p of products.filter(x=>x.active!==false)){let target=p.supplierId?suppliers.find(s=>s.id===p.supplierId):sm.get(normalizeText(p.supplier));const active=byEan.get(p.ean)||[];if(!target&&active.length)target=active.sort((a,b)=>String(b.lastPurchaseAt||b.updatedAt||"").localeCompare(String(a.lastPurchaseAt||a.updatedAt||"")))[0]&&suppliers.find(s=>s.id===active[0].supplierId);if(target&&(active.length!==1||active[0]?.supplierId!==target.id))await setExclusiveSupplierForProduct(target,p.ean,+p.pc||0,"MIGRACAO_V344")
  }
}
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.48.json",{cache:"no-store"});
...
async function setStockMgmtMode(mode){stockMgmtViewMode=["general","unit","diff"].includes(mode)?mode:"general";$("#stockMgmtGeneralPanel")?.classList.toggle("hidden",stockMgmtViewMode!=="general");$("#stockMgmtUnitPanel")?.classList.toggle("hidden",stockMgmtViewMode!=="unit");$("#stockMgmtDiffPanel")?.classList.toggle("hidden",stockMgmtViewMode!=="diff");for(const [id,key] of [["stockMgmtTabGeneral","general"],["stockMgmtTabUnit","unit"],["stockMgmtTabDiff","diff"]]){$("#"+id)?.classList.toggle("active",stockMgmtViewMode===key);$("#"+id)?.classList.toggle("secondary",stockMgmtViewMode!==key)}if(stockMgmtViewMode==="unit"){await populateStockMgmtDeparaUnits();await renderStockManagementByUnit()}if(stockMgmtViewMode==="diff")await renderStockDifferenceAnalysis()}
...
async function renderSupplierRegistryAdmin(){
  ...
  box.innerHTML=suppliers.length?suppliers.map(s=>`<div class="row"><span><b>${esc(s.name)}</b><br><small>${s.active===false?"Inativo":"Ativo"}</small></span><span class="mini"><button type="button" onclick="editSupplierRegistry('${s.id}')">Editar</button><button type="button" class="secondary" onclick="openSupplierProducts('${s.id}')">Ver produtos</button></span></div>`).join(""):'<p class="muted">Nenhum fornecedor cadastrado.</p>';
...
function showPurchaseTab(t){
  const tabs=["weekly","replenishment","manual","nf"];if(!tabs.includes(t))t="weekly";
  ...
  if(t==="weekly")renderWeeklyPurchasePlan();if(t==="replenishment")renderPurchaseFlow()
}
...
async function populatePurchaseFlowFilters(rows){
  const units=[...new Map(rows.map(x=>[x.unitId,x.unit])).entries()].sort((a,b)=>a[1].localeCompare(b[1])),sup=[...new Set(rows.map(x=>x.supplier).filter(Boolean))].sort();
  const us=$("#purchaseFlowUnit"),ss=$("#purchaseFlowSupplier");if(us){const cur=us.value;us.innerHTML='<option value="">Todos</option>'+units.map(([id,n])=>`<option value="${esc(id)}">${esc(n)}</option>`).join("");us.value=cur}if(ss){const cur=ss.value;ss.innerHTML='<option value="">Todos</option>'+sup.map(n=>`<option value="${esc(n)}">${esc(n)}</option>`).join("");ss.value=cur}
}
function purchaseFlowStatusLabel(x){
  if(x.status==="SUPPLIED")return"Abastecido";if(x.status==="DISPATCHED")return"Despachado / pendente";if(x.status==="CD_RESERVED")return"Reservado no CD";if(x.status==="READY")return"Pronto para abastecer";if(x.status==="AWAITING_EXPIRY")return"Aguardando validade";return"Aguardando recebimento"
}
async function renderPurchaseFlow(){
  ...
  const pending=rows.filter(x=>x.status!=="SUPPLIED").reduce((s,x)=>s+Math.max(0,(+x.qty||0)-(+x.suppliedQty||0)),0),ready=rows.filter(x=>["READY","DISPATCHED"].includes(x.status)).reduce((s,x)=>s+Math.max(0,(+x.receivedQty||0)-(+x.suppliedQty||0)),0);
  ...
  const box=$("#purchaseFlowList");if(!box)return;box.innerHTML=rows.length?`<div class="dtable"><table><thead><tr><th>Compra</th><th>Condomínio</th><th>Fornecedor</th><th>Produto</th><th>Qtd.</th><th>Fluxo</th><th>Status</th><th>Validade</th><th>Lote</th><th>Ação</th></tr></thead><tbody>${rows.map(x=>`<tr><td>${esc(x.purchaseId)}</td><td>${esc(x.unit)}</td><td>${esc(x.supplier||"—")}</td><td><b>${esc(x.product)}</b><br><small>${esc(x.ean)}</small></td><td>${qtyFmt(x.qty)}</td><td>${x.route==="CD"?"CD":"Imediato"}</td><td><b>${purchaseFlowStatusLabel(x)}</b></td><td><input type="date" id="pfexp_${x.id}" value="${esc(x.expiry||"")}"></td><td><input id="pflot_${x.id}" value="${esc(x.lot||"")}" placeholder="Lote"></td><td>${purchaseFlowActions(x)}</td></tr>`).join("")}</tbody></table></div>`:'<p class="muted">Nenhuma mercadoria encontrada com os filtros selecionados.</p>'
}
function purchaseFlowActions(x){
  if(x.status==="AWAITING_RECEIPT"||x.status==="AWAITING_EXPIRY")return `<button onclick="receiveWeeklyPurchaseItem('${x.purchaseId}','${x.id}')">Receber + validade</button>`;
  if(x.route==="CD"&&x.status==="CD_RESERVED")return `<button onclick="dispatchWeeklyPurchaseItem('${x.purchaseId}','${x.id}')">Despachar</button>`;
  if(["READY","DISPATCHED"].includes(x.status))return `<button onclick="confirmWeeklySupply('${x.purchaseId}','${x.id}')">Confirmar abastecimento</button>`;
  return "—"
}
async function mutateWeeklyPurchaseItem(pid,itemId,fn){
  const p=await get("purchases",pid);if(!p)return;const it=(p.items||[]).find(x=>x.id===itemId);if(!it)return;await fn(it,p);p.updatedAt=new Date().toISOString();p.status=(p.items||[]).every(x=>x.status==="SUPPLIED")?"COMPLETED":"IN_PROGRESS";await localPut("purchases",p);await renderPurchaseFlow()
}
async function receiveWeeklyPurchaseItem(pid,itemId){
  const exp=$("#pfexp_"+itemId)?.value||"",lot=$("#pflot_"+itemId)?.value.trim()||"";if(!exp)return alert("Registre a validade antes de liberar a mercadoria.");
  await mutateWeeklyPurchaseItem(pid,itemId,async(it,p)=>{it.receivedQty=+it.qty||0;it.expiry=exp;it.lot=lot;it.status=it.route==="CD"?"CD_RESERVED":"READY";it.receivedAt=new Date().toISOString();await audit("COMPRA_RECEBIDA_VALIDADE",{purchaseId:pid,ean:it.ean,unitId:it.unitId,route:it.route,expiry:exp,qty:it.qty})})
}
async function dispatchWeeklyPurchaseItem(pid,itemId){
  await mutateWeeklyPurchaseItem(pid,itemId,async it=>{it.status="DISPATCHED";it.dispatchedQty=+it.receivedQty||+it.qty||0;it.dispatchedAt=new Date().toISOString();await audit("MERCADORIA_DESPACHADA",{purchaseId:pid,ean:it.ean,unitId:it.unitId,qty:it.dispatchedQty})})
}
async function confirmWeeklySupply(pid,itemId){
  await mutateWeeklyPurchaseItem(pid,itemId,async it=>{const q=Math.max(0,(+it.receivedQty||+it.qty||0)-(+it.suppliedQty||0));if(q<=0)return;const p=await prod(it.ean);await adjustStock(it.unitId,it.ean,q,p,+it.cost||0);await upsertLot(it.unitId,it.ean,q,it.expiry||"",it.lot||"",it.cost,pid);if(it.expiry)await localPut("expiries",{id:id("e"),unitId:it.unitId,ean:it.ean,product:it.product,qty:q,date:it.expiry,lot:it.lot||"",purchaseId:pid,updatedAt:new Date().toISOString()});await localPut("moves",{id:id("m"),at:new Date().toISOString(),type:"ABASTECIMENTO_COMPRA",to:it.unitId,ean:it.ean,product:it.product,qty:q,unitCost:it.cost,purchaseId:pid,updatedAt:new Date().toISOString()});it.suppliedQty=(+it.suppliedQty||0)+q;it.status="SUPPLIED";it.suppliedAt=new Date().toISOString();await audit("ABASTECIMENTO_COMPRA_CONFIRMADO",{purchaseId:pid,ean:it.ean,unitId:it.unitId,qty:q})})
}
