...
async function refreshLoginConnectionStatus(showMessage=false){
  ...
  try{
    ...
    if(status.authContract!==AUTH_CONTRACT)throw new Error("Backend incompatível com esta versão. Atualize o Apps Script para 3.23.0.");
...
async function getPhysicalCountUnitProducts(unitId){
  ...
  const activePlanogram=planogram.filter(x=>x.active!==false);
  return activePlanogram.map(pg=>{
    const ean=pg.ean,p=pm.get(ean),s=sm.get(ean),d=dm.get(ean);
    return {ean,product:p?.name||pg.product||s?.product||ean,p:p||null,predicted:Math.round(+s?.qty||0),observed:null,avgCost:+s?.avgCost||+s?.lastCost||0,status:d?.status||"",selected:true}
...
function renderPhysicalCountProductList(){
  ...
  </tr>`).join("")}</tbody></table></div>`:'<p class="muted">Nenhum produto no planograma desta unidade. Vá em Produtos → Planograma por Condomínio / CD e selecione os itens.</p>'
}
...
async function countPhysicalEAN(ean){
  ...
  if(!r){
    ...
    const s=await get("stock",unitId+"|"+clean);r={ean:clean,product:p.name,p,predicted:Math.round(+s?.qty||0),observed:0,avgCost:+s?.avgCost||+s?.lastCost||0,selected:true};physicalCountRows.push(r)
  }
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    const status=await apiGet("status");add("Backend",status.version==="3.23.0",`Versão ${status.version||"?"}`);add("Sessão",!!currentSession?.token,currentSession?.profileName||currentSession?.role||"sem perfil");
...
async function backup(){const o={version:"3.23.0",createdAt:new Date().toISOString(),stores:{}};for(const s of SYNC_STORES)o.stores[s]=await all(s);const b=new Blob([JSON.stringify(o,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="OKEO_CORE_Backup_V3_23.json";a.click()}