...
const FRONTEND_VERSION="5.0.0";
const EXPECTED_BACKEND_VERSION="5.0.0";
...
async function probeBackendUrlDirect(url,timeoutMs=10000){
  ...
  try{
    ...
    const res=await fetch(`${clean}${sep}action=status&_=${Date.now()}`,{
      method:"GET",
      cache:"no-store",
      redirect:"follow",
    ...
    let data;try{data=JSON.parse(text)}catch(e){throw new Error("Resposta inválida da Base Central")}
    if(!data?.ok)throw new Error(data?.error||"Base Central não respondeu OK");
    return data
  }catch(e){
    if(e?.name==="AbortError")throw new Error("TIMEOUT_BASE_CENTRAL");
    if(String(e?.message||e)==="Failed to fetch"){
      throw new Error("FALHA_CORS_OU_REDE_APPS_SCRIPT")
    }
    throw e
...
async function backendRequest(action,params={},method="POST",options={}){
  ...
  const timeoutMs=Math.max(1500,+options.timeoutMs||12000),
  ...
  try{
    ...
    if(e?.name==="AbortError"||String(e).includes("TIMEOUT")){
      const err=new Error("TIMEOUT_BASE_CENTRAL");
      err.code="TIMEOUT_BASE_CENTRAL";
      throw err
    }
    throw e
  }finally{
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=5.0.0",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="5.0.0"){
        localStorage.setItem("okeo_frontend_cache_version","5.0.0")
      }
    })
...
async function verifyBackendVersion(expected="5.0.0"){
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("5.0.0");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 5.0.0.`;
      alert(`Publique primeiro o backend 5.0.0. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    ...
    const resumable=progress&&progress.version==="5.0.0";
    if(!resumable){
      ...
      await backup();
      progress={version:"5.0.0",storeIndex:0,rowIndex:0,sent:0,startedAt:new Date().toISOString()};
...
async function runDeepHealthCheck(){
  ...
  if(remote.ok){
    ...
    add("Backend",status.version==="5.0.0",`publicado v${status.version||"?"} • esperado v5.0.0`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="5.0.0",`Versão ${status.version||"?"}`);
