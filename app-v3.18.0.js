...
function bind(){
  ...
  if($("#controlStockTabQuick"))$("#controlStockTabQuick").onclick=()=>setControlStockMode("quick");
  if($("#controlStockTabPoint"))$("#controlStockTabPoint").onclick=()=>setControlStockMode("point");
}
...
let controlStockMode="quick";
async function renderControlStock(){
  $("#controlStockQuickPanel").classList.toggle("hidden",controlStockMode!=="quick");
  $("#controlStockPointPanel").classList.toggle("hidden",controlStockMode!=="point");
  $("#controlStockTabQuick").classList.toggle("active",controlStockMode==="quick");
  $("#controlStockTabPoint").classList.toggle("active",controlStockMode==="point");
  if(controlStockMode==="quick")await renderStock();else await renderControlPoint()
}
async function setControlStockMode(mode){controlStockMode=mode==="point"?"point":"quick";await renderControlStock()}
// ---------- Produtos ----------
...
async function backup(){const o={version:"3.18.0",createdAt:new Date().toISOString(),stores:{}};for(const s of SYNC_STORES)o.stores[s]=await all(s);const b=new Blob([JSON.stringify(o,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="OKEO_CORE_Backup_V3_18.json";a.click()}