...
function bind(){
  ...
  if($("#productUnitToggle"))if($("#productUnitToggle"))if($("#productUnitToggle"))$("#productUnitToggle").onclick=async()=>{await ensureReferenceDataAvailable("CADASTRO_PRODUTO_UNIDADES");await renderProductUnitChecklist();$("#productUnitDropdown")?.classList.toggle("hidden")};
  if($("#productUnitSearch"))if($("#productUnitSearch"))if($("#productUnitSearch"))$("#productUnitSearch").oninput=renderProductUnitChecklist;
  if($("#productUnitSelectAll"))if($("#productUnitSelectAll"))if($("#productUnitSelectAll"))$("#productUnitSelectAll").onclick=selectAllProductUnits;
  if($("#productUnitClearAll"))if($("#productUnitClearAll"))if($("#productUnitClearAll"))$("#productUnitClearAll").onclick=clearAllProductUnits;
  ...
  if($("#repDraft"))if($("#repDraft"))if($("#repDraft"))if($("#repDraft"))$("#repDraft").onclick=generateReplenishmentDraft;if($("#repApprove"))if($("#repApprove"))if($("#repApprove"))if($("#repApprove"))$("#repApprove").onclick=approveReplenishment;if($("#cfinalize"))$("#cfinalize").onclick=finalizePhysicalCount;if($("#cadd"))$("#cadd").onclick=addControlItem;
  if($("#repUnitsToggle"))if($("#repUnitsToggle"))if($("#repUnitsToggle"))if($("#repUnitsToggle"))$("#repUnitsToggle").onclick=()=>$("#repUnitsPanel")?.classList.toggle("hidden");if($("#repSelectAll"))if($("#repSelectAll"))if($("#repSelectAll"))if($("#repSelectAll"))$("#repSelectAll").onclick=()=>selectAllRepUnits();if($("#repClearUnits"))if($("#repClearUnits"))if($("#repClearUnits"))if($("#repClearUnits"))$("#repClearUnits").onclick=()=>{repSelectedUnits.clear();renderRepUnitChecks()};if($("#repUnitSearch"))if($("#repUnitSearch"))if($("#repUnitSearch"))if($("#repUnitSearch"))$("#repUnitSearch").oninput=renderRepUnitChecks;if($("#createUser"))$("#createUser").onclick=saveUserAdmin;if($("#changeMyPassword"))$("#changeMyPassword").onclick=changeOwnPassword;
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=3.79.0",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="3.79.0"){
        localStorage.setItem("okeo_frontend_cache_version","3.79.0")
      }
    })
    ...
    if(diag){
      ...
      if(lp>0&&(cp===0||lp>cp*2)&&lu>=cu){
        ...
        const go=confirm(
          ...
          `Para sincronizar computador e celular, primeiro precisamos publicar a base local completa na Base Central.\n\n`+
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.79.json",{cache:"no-store"});
...
async function publishLocalBaseToCentral(){
  ...
  const answer=prompt(
    `Esta operação vai PUBLICAR os dados deste dispositivo na Base Central.\n\n`+
    `Local: ${diag.localTotal} registros\nCentral: ${diag.centralTotal} registros\n\n`+
    `Ela não apaga registros centrais: registros com o mesmo ID serão atualizados e os demais serão adicionados.\n\n`+
    `Digite PUBLICAR para continuar.`
  );
  if(answer!=="PUBLICAR")return;
  ...
  try{
    ...
    // esvazia fila antiga apenas após upload completo; os registros já estão no Central.
    ...
    if(status)status.textContent=`Publicação concluída: ${sent} registro(s) enviados para a Base Central.`;
    await diagnoseDeviceSync();
    alert("Base Central publicada. Agora, no celular, use Configurações → Baixar Base Central neste dispositivo.")
  }catch(e){
    ...
    if(btn){btn.disabled=false;btn.textContent="Publicar esta base local na Base Central"}
  }
}
async function downloadCentralBaseToDevice(){
  ...
  try{
    let loaded=0;
    // Não limpa tudo de uma vez: cada store só é substituído após resposta válida do servidor.
    for(const store of SYNC_STORES){
    ...
    alert("Carga interrompida: "+e.message)
  }finally{
