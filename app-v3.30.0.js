...
async function renderStockManagementByUnit(){const [units,products,stock,demand]=await Promise.all([all("units"),all("products"),all("stock"),all("demandCurrent")]),activeUnits=units.filter(u=>u.active!==false).sort((a,b)=>a.name.localeCompare(b.name)),sel=$("#stockMgmtUnitSelect"),current=sel?.value||activeUnits.find(u=>u.type!=="CD")?.id||activeUnits[0]?.id||"";if(sel){sel.innerHTML=activeUnits.map(u=>`<option value="${u.id}">${esc(u.name)}</option>`).join("");if(current&&activeUnits.some(u=>u.id===current))sel.value=current}const unitId=sel?.value;if(!unitId)return;const pm=new Map(products.map(p=>[p.ean,p])),sm=new Map(stock.filter(s=>s.unitId===unitId).map(s=>[s.ean,s])),dm=new Map(demand.filter(d=>d.unitId===unitId).flatMap(d=>(d.eans||[d.ean]).filter(Boolean).map(e=>[e,d]))),plan=(await byIndex("planograms","unitId",unitId)).filter(x=>x.active!==false),q=normalizeText($("#stockMgmtUnitSearch")?.value||""),rows=plan.map(pg=>{const ean=pg.ean,p=pm.get(ean)||{},s=sm.get(ean)||{},d=dm.get(ean)||{},avg=+d.averageWeekly||+d.weeklyAverage||0;return{ean,product:p.name||pg.product||ean,stock:Math.round(+s.qty||0),peak:Math.ceil(+d.peakDemand||+d.peakWeekly||+d.maxWeekly||avg||0),ideal:Math.ceil(+d.idealStock||avg||0),alert:Math.ceil(+d.alertLevel||avg*.3||0),status:d.status||""}}).filter(r=>!q||normalizeText([r.product,r.ean].join(" ")).includes(q));$("#stockMgmtUnitTable").innerHTML=rows.length?`<div class="dtable"><table><thead><tr><th>Produto</th><th>EAN</th><th>Estoque atual</th><th>Pico de demanda</th><th>Recomendado em estoque</th><th>Nível de alerta</th><th>Status</th></tr></thead><tbody>${rows.map(r=>`<tr><td><b>${esc(r.product)}</b></td><td>${r.ean}</td><td>${qtyFmt(r.stock)}</td><td>${qtyFmt(r.peak)}</td><td>${qtyFmt(r.ideal)}</td><td>${qtyFmt(r.alert)}</td><td>${esc(r.status)}</td></tr>`).join("")}</tbody></table></div>`:'<p class="muted">Nenhum produto no planograma desta unidade.</p>'}
async function renderExpiryPlanogramProducts(){const unitId=$("#eunit")?.value,box=$("#expiryPlanogramProducts");if(!box||!unitId)return;const [plan,products]=await Promise.all([byIndex("planograms","unitId",unitId),all("products")]),pm=new Map(products.map(p=>[p.ean,p])),q=normalizeText($("#expiryPlanogramSearch")?.value||""),rows=plan.filter(x=>x.active!==false).map(x=>pm.get(x.ean)||{ean:x.ean,name:x.product||x.ean}).filter(p=>!q||normalizeText([p.name,p.ean].join(" ")).includes(q)).sort((a,b)=>String(a.name||"").localeCompare(String(b.name||"")));box.innerHTML=rows.length?rows.map(p=>`<button type="button" class="expiry-plan-row" onclick="selectExpiryPlanogramProduct('${p.ean}')"><span><b>${esc(p.name)}</b><small>EAN ${p.ean}</small></span></button>`).join(""):'<p class="muted">Nenhum produto no planograma desta unidade.</p>'}
...
async function saveExpiry(){
  ...
  if(available<=0){
    ...
    if(confirm(`O sistema registra estoque 0 para ${p.name}. Se o produto está fisicamente no local, corrija primeiro pela Contagem de Estoque Física. Deseja abrir Controle de Estoque?`))view("controlstock");
    return
  }
