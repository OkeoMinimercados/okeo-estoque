...
const SYNC_STORES=["products","units","stock","expiries","moves","groups","salesWeekly","salesImports","invoices","ruptureEvents","demandBase","replenishments","controlPoints","controlPointItems","purchases","lots","auditLog","supplierOffers","suppliers"];
const SYNC_SCOPES={
  CORE:["products","units","stock","expiries","groups","demandBase","replenishments","controlPoints","controlPointItems","lots","supplierOffers","suppliers"],
...
async function init(){
  ...
  if(saved){
    try{
      ...
      if(validated?.valid){currentSession={...s,...validated.user,token:s.token};persistSession(currentSession,!!localStorage.getItem("okeo_session"));await showApp();return}
...
async function showApp(){
  ...
  try{await bootstrapFromCentral()}catch(e){console.warn("Bootstrap central",e)}
  try{await migrateLocalV31()}catch(e){console.warn("Migração V3.1",e)}
  try{await syncScope("CORE",false)}catch(e){console.warn("Sincronização Core",e)}
  try{await selectors()}catch(e){console.warn("Seletores",e)}
  applyAccessProfile();
...
async function login(){
  ...
  try{
    ...
    currentSession={token:r.token,username:r.username,displayName:r.displayName||r.username,role:r.role||"EMPLOYEE",profileId:r.profileId||"",profileName:r.profileName||"",permissions:r.permissions||[],expiresAt:r.expiresAt};
...
function persistSession(s,remember){sessionStorage.removeItem("okeo_session");localStorage.removeItem("okeo_session");(remember?localStorage:sessionStorage).setItem("okeo_session",JSON.stringify(s))}
...
async function validateSession(token){
  if(!getBackendUrl())return {valid:false};
  try{return await authPost("validate_session",{token})}catch(e){return {valid:false}}
}
async function view(v){
  if(!canView(v)){const fallback=firstAllowedView();if(v!==fallback)return view(fallback)}
  const target=$("#"+v);if(!target)return;
  $$(".view").forEach(x=>x.classList.add("hidden"));target.classList.remove("hidden");
  ...
  const scope=VIEW_SYNC_SCOPE[v];if(scope){try{target.insertAdjacentHTML("afterbegin",'<div id="scopeLoading" class="loading-skeleton">Atualizando dados deste módulo…</div>');await syncScope(scope,false)}catch(e){console.warn("Sincronização do módulo",v,e)}finally{target.querySelector("#scopeLoading")?.remove()}}
  ...
  try{if(fn)await fn()}catch(e){console.error("Erro na tela",v,e);target.innerHTML=`<div class="alert-card alert-danger"><b>Não foi possível carregar esta tela.</b><br><small>${esc(e.message||e)}</small></div>`}
}
...
function applyAccessProfile(){
  if(currentSession&&!currentSession.role)currentSession.role="ADMIN"; // migração de sessão antiga
...
async function buildOperationalContext(){
  const [products,units,stock,demandBase,replenishments,groups]=await Promise.all(["products","units","stock","demandBase","replenishments","groups"].map(all));
  ...
  return {products,units,stock,demandBase,replenishments,groups,productByEan,unitById,stockByKey,demandByUnit,groupEans,inboundByKey}
}
function needFromContext(ctx,unitId,ean){
  const p=ctx.productByEan.get(ean),eans=p?.groupId?(ctx.groupEans.get(p.groupId)||new Set([ean])):new Set([ean]),ds=(ctx.demandByUnit.get(unitId)||[]).filter(x=>eans.has(x.ean));
  ...
  if(!ds.length)return {stock,inbound,ideal:0,alert:0,need:0,status:"SEM DEMANDA",groupId:p?.groupId||""};
  const ideal=Math.ceil(ds.reduce((s,x)=>s+(+x.idealStock||+x.averageWeekly||0),0)),alert=Math.ceil(ds.reduce((s,x)=>s+(+x.alertLevel||(+x.averageWeekly||0)*.5),0)),projected=stock+inbound,status=projected<=0?"RUPTURA":projected<=alert?"REPOSIÇÃO":"OK";
  return {stock,inbound,ideal,alert,status,need:status==="OK"?0:Math.max(0,ideal-projected),groupId:p?.groupId||""}
}
...
async function savePurchase(){
  if(!purchaseDraftItems.length)return alert("Adicione itens.");const now=new Date().toISOString(),pid="COMP-"+now.slice(0,10).replaceAll("-","")+"-"+String(Date.now()).slice(-5);
  const purchaseSupplierName=$("#purchaseSupplier").value.trim(),purchaseSupplier=await ensureSupplier(purchaseSupplierName),purchase={id:pid,at:now,date:$("#purchaseDate").value||now.slice(0,10),supplier:purchaseSupplierName,supplierId:purchaseSupplier?.id||"",document:$("#purchaseDoc").value.trim(),items:structuredClone(purchaseDraftItems),nf:purchaseNFData,status:"RECEIVED_AND_DISTRIBUTED",updatedAt:now,user:currentSession?.username||""};
  for(const it of purchaseDraftItems){const p=await prod(it.ean);if(!p)continue;for(const d of it.distribution.filter(x=>+x.qty>0)){const q=+d.qty;await adjustStock(d.unitId,it.ean,q,p,+it.cost||0);await localPut("moves",{id:id("m"),at:now,type:d.status==="CD"?"COMPRA_CD":"COMPRA_DISTRIBUIDA",to:d.unitId,ean:it.ean,product:p.name,qty:q,unitCost:it.cost,purchaseId:pid,updatedAt:now});if(it.expiry){await localPut("expiries",{id:id("e"),unitId:d.unitId,ean:it.ean,product:p.name,qty:q,date:it.expiry,lot:it.lot||"",purchaseId:pid,updatedAt:now});await upsertLot(d.unitId,it.ean,q,it.expiry,it.lot||"",it.cost,pid)}}}
  await localPut("purchases",purchase);for(const it of purchase.items)await recordSupplierOffer(it.supplier||purchase.supplier,it.ean,it.cost);await audit("COMPRA_CONFIRMADA",{purchaseId:pid,supplier:purchase.supplier,total:purchase.items.reduce((s,x)=>s+(+x.total||0),0),items:purchase.items.length});await syncAll(false);$("#purchaseMsg").textContent=`${pid} registrada; estoques atualizados.`;await clearPurchaseDraft();renderEntries()
}
...
async function saveExpiry(){
  ...
  if(!p)return alert("Produto não cadastrado.");if(!date)return alert("Informe validade.");if(!u)return alert("Selecione a unidade.");
  const row={id:id("e"),unitId:u,ean:e,product:p.name,date,qty:q,lot,photo:await fileData($("#ephoto").files[0]),updatedAt:new Date().toISOString()};
  await localPut("expiries",row);await upsertLot(u,e,q,date,lot,0,"VALIDADE_MANUAL");await audit("VALIDADE_REGISTRADA",{unitId:u,ean:e,qty:q,date,lot});renderExpiry()
}
...
async function importSales(){
  ...
  const rows=csv(text),units=await all("units"),products=await all("products"),bases=await all("demandBase"),nameMap=new Map(),unitNameMap=new Map();for(const un of units)for(const nm of [un.name,...(un.aliases||[])])unitNameMap.set(normalizeText(nm),un);
  ...
  for(const x of rows){
    const date=x.data||x.datahora,productName=String(x.produto||"").trim(),pr=nameMap.get(normalizeText(productName)),un=unitNameMap.get(normalizeText(x.local||x.unidade||x.condominio||""));
...
async function calcDemand(){
  const [p,units,sw,g,st,base]=await Promise.all(["products","units","salesWeekly","groups","stock","demandBase"].map(all)),scope=unitScope($("#dunit").value,units),windowWeeks=+$("#dweeks").value,pm=new Map(p.map(x=>[x.ean,x])),gm=new Map(g.map(x=>[x.id,x])),now=new Date();
  const groups={};for(const pr of p.filter(x=>x.active!==false)){const k=pr.groupId?"g:"+pr.groupId:"p:"+pr.ean,label=pr.groupId?gm.get(pr.groupId)?.name||pr.name:pr.name;groups[k]=groups[k]||{label,eans:new Set(),histQty:0,histWeeks:0,histPeak:0,newWeeks:{}};groups[k].eans.add(pr.ean)}
  for(const b of base.filter(x=>scope.has(x.unitId))){const pr=pm.get(b.ean);if(!pr)continue;const k=pr.groupId?"g:"+pr.groupId:"p:"+pr.ean,x=groups[k];if(!x)continue;const start=b.periodStart?new Date(b.periodStart):null,end=b.periodEnd?new Date(b.periodEnd):null,weeks=start&&end?Math.max(1,Math.round((end-start)/604800000)+1):Math.max(1,+b.historicalWeeks||12);x.histQty+=(+b.historicalQty||(+b.averageWeekly||0)*weeks);x.histWeeks=Math.max(x.histWeeks,weeks);x.histPeak=Math.max(x.histPeak,+b.peakWeekly||0)}
  const baseEnd=base.map(x=>x.periodEnd).filter(Boolean).sort().pop()||"";
  for(const x of sw){if(!scope.has(x.unitId))continue;if(baseEnd&&x.week<=baseEnd)continue;const pr=pm.get(x.ean);if(!pr)continue;const k=pr.groupId?"g:"+pr.groupId:"p:"+pr.ean;if(!groups[k])continue;groups[k].newWeeks[x.week]=(groups[k].newWeeks[x.week]||0)+(+x.qty||0)}
  const result=[],unitView=$("#dunit").value;
  for(const [key,x] of Object.entries(groups)){
    const newVals=Object.values(x.newWeeks),newQty=newVals.reduce((s,n)=>s+n,0),newWeeks=Math.max(0,Object.keys(x.newWeeks).length),den=Math.max(1,x.histWeeks+newWeeks),avg=(x.histQty+newQty)/den,peak=Math.max(x.histPeak,0,...newVals);
    const stocks=st.filter(s=>scope.has(s.unitId)&&x.eans.has(s.ean)),calculated=Math.max(0,stocks.reduce((s,n)=>s+(+n.qty||0),0)),alert=Math.ceil(avg*.5),ideal=Math.ceil(avg),status=calculated<=0?"RUPTURA":calculated<=alert?"REPOSIÇÃO":"OK",replenish=status==="OK"?0:Math.max(0,ideal-calculated),coverage=daysCoverage(calculated,avg),ruptureIn=projectedRupture(calculated,avg),excess=Math.max(0,calculated-ideal);
    await recordRupture(key,x.label,unitView,calculated);result.push({...x,avg,peak,alert,ideal,calculated,status,replenish,coverage,ruptureIn,excess,histQty:x.histQty,newQty})
  }
  result.sort((a,b)=>({RUPTURA:0,"REPOSIÇÃO":1,OK:2}[a.status]-{RUPTURA:0,"REPOSIÇÃO":1,OK:2}[b.status])||b.replenish-a.replenish);
  $("#dlist").innerHTML=`<div class="dtable"><table><thead><tr><th>Produto/Grupo</th><th>Estoque atual</th><th>Alerta</th><th>Ideal</th><th>Média semanal</th><th>Pico</th><th>Cobertura</th><th>Ruptura estimada</th><th>Excesso</th><th>Status</th><th>Repor</th></tr></thead><tbody>${result.map(x=>`<tr><td>${esc(x.label)}</td><td>${n2(x.calculated)}</td><td>${x.alert}</td><td>${x.ideal}</td><td>${n2(x.avg)}</td><td>${n2(x.peak)}</td><td class="${x.coverage==null?"":x.coverage<3?"coverage-bad":x.coverage<7?"coverage-warn":"coverage-good"}">${x.coverage==null?"—":n2(x.coverage)+" dias"}</td><td>${x.ruptureIn==null?"—":x.ruptureIn+" dias"}</td><td>${n2(x.excess)}</td><td><b class="${x.status==="RUPTURA"?"bad":x.status==="REPOSIÇÃO"?"warn":"ok"}">${x.status}</b></td><td><b>${n2(x.replenish)}</b></td></tr>`).join("")}</tbody></table></div>`
}
...
async function generateReplenishmentDraft(){
  ...
  const offerByEan=new Map();for(const o of(await all("supplierOffers")).filter(x=>x.active!==false)){const a=offerByEan.get(o.ean)||[];a.push(o);offerByEan.set(o.ean,a)}
  repDraftRows=[];
  for(const u of ctx.units.filter(x=>repSelectedUnits.has(x.id))){
    const keys=new Map();for(const d of(ctx.demandByUnit.get(u.id)||[])){const p=ctx.productByEan.get(d.ean);if(!p||p.active===false)continue;const key=p.groupId?"g:"+p.groupId:"p:"+p.ean;if(!keys.has(key))keys.set(key,[]);keys.get(key).push(p.ean)}
    for(const [key,raw] of keys){
      const eans=[...new Set(raw)],n=needFromContext(ctx,u.id,eans[0]);let need=n.need;if(need<=0)continue;
      ...
      if(fromCd>0){repDraftRows.push({id:id("ri"),unitId:u.id,unit:u.name,ean:selected,product:p.name,suggestedQty:fromCd,finalQty:fromCd,originType:"CD",originUnitId:cd?.id||"",supplier:"",stock:n.stock,ideal:n.ideal,status:n.status,warning:await repExpiryWarning(u.id,selected),receivedQty:0,executedQty:0,note:"",groupKey:key});cdAvail[selected]-=fromCd;need-=fromCd}
      if(need>0){const candidates=[];for(const ean of eans){const pr=ctx.productByEan.get(ean),offers=offerByEan.get(ean)||[];if(offers.length)for(const o of offers)candidates.push({ean,product:pr,cost:+o.lastCost||0,supplier:o.supplierName||""});else candidates.push({ean,product:pr,cost:0,supplier:pr?.supplier||""})}candidates.sort((a,b)=>(a.cost||1e99)-(b.cost||1e99));const best=candidates[0];selected=best?.ean||eans[0];p=best?.product||ctx.productByEan.get(selected);repDraftRows.push({id:id("ri"),unitId:u.id,unit:u.name,ean:selected,product:p?.name||"",suggestedQty:need,finalQty:need,originType:"COMPRA",originUnitId:"",supplier:best?.supplier||p?.supplier||"",estimatedCost:+best?.cost||0,stock:n.stock,ideal:n.ideal,status:n.status,warning:await repExpiryWarning(u.id,selected),receivedQty:0,executedQty:0,note:"",groupKey:key})}
    }
  }
  renderReplenishmentEditor()
}
async function renderReplenishmentEditor(){
  const units=(await all("units")).filter(x=>x.active!==false),opts=units.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join("");
...
async function disableUser(username,active){await apiPost("set_user_active",{username,active:String(!active)});renderUsersAdmin()}
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    for(const f of ["renderSales","calcDemand","generateReplenishmentDraft","approveReplenishment","renderReplenishment","startControlPoint","approveControlPoint","savePurchase","renderUsersAdmin","saveProfileAdmin","runIntegrityCheck"])add("Função "+f,typeof window[f]==="function",typeof window[f]==="function"?"OK":"Não carregada");
    ...
    const status=await apiGet("status");add("Backend",status.version==="3.3.0",`Versão ${status.version||"?"}`);add("Sessão",!!currentSession?.token,currentSession?.profileName||currentSession?.role||"sem perfil");
