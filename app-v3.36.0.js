...
let currentSession=null,repSelectedUnits=new Set(),productUnitSelection=new Set();
...
async function view(v){
  ...
  const titles={home:["Dashboard","Gestão operacional OKEO"],stockmgmt:["Gestão de Estoque","Valor, quantidade, validade e movimentos"],products:["Produtos","Cadastro Mestre"],units:["Unidades","Condomínios, mercados e CD"],controlstock:["Contagem de Estoque","Contagem de Estoque Física por planograma"],entries:["Compras","Planejamento semanal, reposição, NF e distribuição"],moves:["Ajuste de Estoque","Perdas, ajustes e transferências manuais"],cdmove:["Movimentação Estoque CD","Saldo do CD e sugestões de alocação"],expiry:["Validades","Produtos próximos do vencimento"],groups:["Grupos","Produtos substituíveis"],sales:["Vendas","Importação e histórico"],demand:["Demanda Inteligente","Estoque ideal e alertas"],settings:["Configurações","Usuários, perfis e integrações"]};
...
async function syncPhysicalCountFromManualEntry(ean,value){
  ...
  if(!r){const cp=activeControlId?await get("controlPoints",activeControlId):null,unitId=cp?.unitId||$("#cunit")?.value||"",s=unitId?await get("stock",unitId+"|"+clean):null;r={ean:clean,product:p.name,p,predicted:Math.round(+s?.qty||0),observed:qty,avgCost:+s?.avgCost||+s?.lastCost||+p.pc||0,selected:true,inPlanogram:!!(unitId&&await get("planograms",unitId+"|"+clean))};physicalCountRows.push(r)}else{r.observed=qty;r.selected=true}
...
async function getPhysicalCountUnitProducts(unitId){
  const [products,stock,demand,planogram]=await Promise.all([all("products"),all("stock"),all("demandCurrent"),byIndex("planograms","unitId",unitId)]),
...
async function loadPhysicalCountProducts(reset=false){
  ...
  const notice=$("#physicalCountPlanogramNotice");if(notice)notice.innerHTML=physicalCountRows.length?`<div class="alert-card alert-info"><b>${qtyFmt(physicalCountRows.length)} produto(s)</b> cadastrados no planograma da unidade selecionada. A contagem abaixo usa exclusivamente esta lista.</div>`:`<div class="alert-card alert-warning"><b>Esta unidade não possui lista de produtos cadastrada.</b> Envie o DE/PARA Produto × Condomínio ou importe em Produtos → Planograma / Gestão de Estoque para atualizar a Base Mestre operacional.</div>`;
...
async function readStockMgmtDeparaFile(file){
  ...
  return deparaRowsFromMatrix(parseDelimitedRows(await file.text()),file.name)
}
...
async function applyStockMgmtDepara(){
  ...
  for(const x of valid){const id=x.unit.id+"|"+x.product.ean,old=await get("planograms",id);await localPut("planograms",{...(old||{}),id,unitId:x.unit.id,ean:x.product.ean,product:x.product.name,active:true,source:"DEPARA_IMPORT",updatedAt:now})}
...
async function renderStockManagementByUnit(){
  ...
  const plan=(await byIndex("planograms","unitId",unitId)).filter(x=>x.active!==false),planMap=new Map(plan.map(x=>[x.ean,x]));
  const relevantEans=new Set(plan.map(x=>x.ean)),outsideEans=new Set();
  ...
  if($("#stockMgmtUnitNotice"))$("#stockMgmtUnitNotice").innerHTML=!plan.length?`<div class="integrity-warn"><b>Este condomínio ainda não possui lista de produtos cadastrada.</b> Importe um DE/PARA Produto × Condomínio ou cadastre o planograma em Produtos → Produtos por Condomínio.</div>`:outsideEans.size?`<div class="integrity-warn">Há <b>${qtyFmt(outsideEans.size)}</b> produto(s) com saldo/demanda fora da lista oficial deste condomínio. Revise o cadastro para manter todos os módulos alinhados.</div>`:"";
  $("#stockMgmtUnitTable").innerHTML=rows.length?`<div class="dtable"><table><thead><tr><th>Produto</th><th>EAN</th><th>Quantidade</th><th>Validade cadastrada</th><th>Próxima validade</th><th>Status abastecimento</th><th>Qtd. sugerida</th></tr></thead><tbody>${rows.map(r=>`<tr class="${r.status==="RUPTURA"?"need-high":r.status==="REPOSIÇÃO"?"need-mid":""}"><td><b>${esc(r.product)}</b></td><td>${r.ean}</td><td><b>${qtyFmt(r.stock)}</b></td><td><b class="${r.hasExpiry?"ok":"warn"}">${r.hasExpiry?"SIM":"NÃO"}</b></td><td>${r.expiry||"—"}</td><td><b class="${r.status==="RUPTURA"?"bad":r.status==="REPOSIÇÃO"?"warn":r.status==="OK"?"ok":""}">${r.status==="OK"?"NÃO PRECISA":r.status==="REPOSIÇÃO"?"PRECISA REPOSIÇÃO":esc(r.status)}</b></td><td>${r.status==="RUPTURA"||r.status==="REPOSIÇÃO"?`<b>${qtyFmt(r.need)}</b>`:"—"}</td></tr>`).join("")}</tbody></table></div>`:plan.length?'<p class="muted">Nenhum produto encontrado com os filtros selecionados.</p>':'<p class="muted">Nenhum produto cadastrado para esta unidade. Use a importação DE/PARA acima.</p>'
}
async function renderExpiryPlanogramProducts(){
  const unitId=$("#eunit")?.value,box=$("#expiryPlanogramProducts");if(!box||!unitId)return;
  const [plan,products,stock,lots]=await Promise.all([byIndex("planograms","unitId",unitId),all("products"),byIndex("stock","unitId",unitId),all("lots")]),pm=new Map(products.map(p=>[p.ean,p])),sm=new Map(stock.map(s=>[s.ean,s])),q=normalizeText($("#expiryPlanogramSearch")?.value||"");
...
async function renderProductUnitChecklist(){
  ...
  if($("#productUnitCount"))$("#productUnitCount").textContent=`${productUnitSelection.size} selecionado(s)`;
}
...
async function loadProductUnitSelection(ean){productUnitSelection.clear();if(ean){for(const r of(await all("planograms")).filter(x=>x.ean===ean&&x.active!==false))productUnitSelection.add(r.unitId)}await renderProductUnitChecklist()}
