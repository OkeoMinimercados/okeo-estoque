...
const EXPECTED_BACKEND_VERSION="4.1.6";
...
async function resolveBackendUrl(){
  const official=DEFAULT_BACKEND_URL;
  let saved="";
  try{
    const local=String(localStorage.getItem(BACKEND_STORAGE_KEY)||"").trim(),
          db=await get("settings","backend"),
          dbUrl=String(db?.url||"").trim();
    saved=local||dbUrl
  }catch(e){}
  const customAllowed=localStorage.getItem("okeo_backend_custom_allowed")==="1";
  let url=customAllowed&&saved?saved:official;
  if(!url)url=official;
  localStorage.setItem(BACKEND_STORAGE_KEY,url);
  try{await put("settings",{id:"backend",url,customAllowed})}catch(e){}
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=4.1.6",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="4.1.6"){
        localStorage.setItem("okeo_frontend_cache_version","4.1.6")
      }
    })
...
async function checkBackendCompatibility(force=false){
  ...
  try{
    const s=await backendRequest("status",{},"GET",{timeoutMs:7000});
    backendCompatibility={checkedAt:now,version:String(s.version||""),ok:String(s.version||"")===EXPECTED_BACKEND_VERSION,productionMode:!!s.productionMode,snapshotEpoch:String(s.snapshotEpoch||"")};
    if(backendCompatibility.ok){
      localStorage.setItem("okeo_backend_last_ok_version",backendCompatibility.version);
      localStorage.setItem("okeo_backend_last_ok_at",new Date().toISOString())
    }
  }catch(e){
    backendCompatibility={checkedAt:now,version:"",ok:false,productionMode:false,snapshotEpoch:"",error:String(e.message||e)}
  }
...
async function verifyBackendVersion(expected="4.1.6"){
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("4.1.6");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 4.1.6.`;
      alert(`Publique primeiro o backend 4.1.6. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    ...
    const resumable=progress&&progress.version==="4.1.6";
    if(!resumable){
      ...
      await backup();
      progress={version:"4.1.6",storeIndex:0,rowIndex:0,sent:0,startedAt:new Date().toISOString()};
...
async function runDeepHealthCheck(){
  ...
  if(remote.ok){
    ...
    add("Backend",status.version==="4.1.6",`publicado v${status.version||"?"} • esperado v4.1.6`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="4.1.6",`Versão ${status.version||"?"}`);
