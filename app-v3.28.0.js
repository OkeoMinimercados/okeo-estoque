...
async function captureStockSnapshot(reason="MANUAL",sourceMeta={}){
  const [stock,units]=await Promise.all([all("stock"),all("units")]),capturedAt=new Date().toISOString();
  const totalQty=stock.reduce((s,x)=>s+Math.round(+x.qty||0),0),totalValue=stock.reduce((s,x)=>s+Math.round(+x.qty||0)*(+x.avgCost||+x.lastCost||0),0);
  const byUnit=units.filter(u=>u.active!==false).map(u=>{const rows=stock.filter(x=>x.unitId===u.id);return{unitId:u.id,unitName:u.name,qty:rows.reduce((s,x)=>s+Math.round(+x.qty||0),0),value:rows.reduce((s,x)=>s+Math.round(+x.qty||0)*(+x.avgCost||+x.lastCost||0),0),skus:rows.filter(x=>+x.qty>0).length}});
  const snapshot={id:"SNAP_"+capturedAt.replace(/\D/g,""),capturedAt,reason,totalQty,totalValue,skuCount:stock.filter(x=>+x.qty>0).length,byUnit,sourceMeta};
  await localPut("stockSnapshots",snapshot);await setOperationalMeta("LAST_STOCK_SNAPSHOT",{id:snapshot.id,capturedAt,reason,totalQty,totalValue});
  const snaps=(await all("stockSnapshots")).sort((a,b)=>String(b.capturedAt).localeCompare(String(a.capturedAt)));
  for(const x of snaps.slice(60))await del("stockSnapshots",x.id);
  return snapshot
}
...
async function renderStockHistory(){
  ...
  if($("#stockHistoryTable"))$("#stockHistoryTable").innerHTML=snaps.length?`<div class="dtable"><table><thead><tr><th>Data/hora</th><th>Motivo</th><th>Quantidade</th><th>SKUs</th><th>Valor</th></tr></thead><tbody>${snaps.map(x=>`<tr><td>${new Date(x.capturedAt).toLocaleString("pt-BR")}</td><td>${esc(x.reason||"")}</td><td>${qtyFmt(x.totalQty)}</td><td>${qtyFmt(x.skuCount)}</td><td>${money(x.totalValue)}</td></tr>`).join("")}</tbody></table></div>`:'<p class="muted">Nenhuma posição histórica registrada.</p>'
}
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    const status=await apiGet("status");add("Backend",status.version==="3.28.0",`Versão ${status.version||"?"}`);add("Sessão",!!currentSession?.token,currentSession?.profileName||currentSession?.role||"sem perfil");
...
async function backup(){const o={version:"3.28.0",createdAt:new Date().toISOString(),stores:{}};for(const s of SYNC_STORES)o.stores[s]=await all(s);const b=new Blob([JSON.stringify(o,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="OKEO_CORE_Backup_V3_28.json";a.click()}