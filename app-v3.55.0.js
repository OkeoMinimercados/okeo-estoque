...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=3.55.0",{updateViaCache:"none"});
    await reg.update();
...
async function renderCdInboundPurchases(){
  ...
  const supplierOld=$("#cdInboundSupplier")?.value||"",unitOld=$("#cdInboundUnit")?.value||"",status=$("#cdInboundStatus")?.value||"",q=normalizeText($("#cdInboundSearch")?.value||"");
  ...
  if(ss){ss.innerHTML='<option value="">Todos</option>'+suppliers.map(n=>`<option value="${esc(n)}">${esc(n)}</option>`).join("");if(suppliers.includes(supplierOld))ss.value=supplierOld}
  if(us){us.innerHTML='<option value="">Todos</option>'+units.map(([id,n])=>`<option value="${esc(id)}">${esc(n)}</option>`).join("");if(units.some(([id])=>id===unitOld))us.value=unitOld}
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.55.json",{cache:"no-store"});
...
function renderPhysicalCountProductList(){
  ...
  ${rows.length?`<div class="dtable physical-count-table"><table><thead><tr><th>✓</th><th>Produto</th><th>EAN</th><th>Previsto</th><th>−</th><th>Contagem</th><th>+</th><th>Diferença</th><th>Validade</th></tr></thead><tbody>${rows.map(r=>physicalCountRowHtml(r)).join("")}</tbody></table></div>`:'<p class="muted">Nenhum produto encontrado na pesquisa.</p>'}`
}
function physicalCountRowHtml(r){
  const lots=Array.isArray(r.expiryLots)?r.expiryLots:[],has=!!r.hasExpiry,dbl=!!r.doubleExpiry,lot1=lots[0]||{},lot2=lots[1]||{};
  const expiryPanel=has?`<div class="physical-expiry-panel">
    <label class="inline-check"><input type="checkbox" ${dbl?"checked":""} onchange="setPhysicalDoubleExpiry('${r.ean}',this.checked)"> Produto tem dupla validade</label>
    <div class="physical-expiry-grid">
      <label>Qtd. 1<input type="number" min="0" step="1" value="${lot1.qty??(dbl?"":r.observed??"")}" onchange="updatePhysicalExpiryField('${r.ean}',0,'qty',this.value)"></label>
      <label>1ª validade<input type="date" value="${esc(lot1.expiry||"")}" onchange="updatePhysicalExpiryField('${r.ean}',0,'expiry',this.value)"></label>
      <label>Lote 1<input value="${esc(lot1.lot||"")}" placeholder="Opcional" onchange="updatePhysicalExpiryField('${r.ean}',0,'lot',this.value)"></label>
      ${dbl?`<label>Qtd. 2<input type="number" min="0" step="1" value="${lot2.qty??""}" onchange="updatePhysicalExpiryField('${r.ean}',1,'qty',this.value)"></label>
      <label>2ª validade<input type="date" value="${esc(lot2.expiry||"")}" onchange="updatePhysicalExpiryField('${r.ean}',1,'expiry',this.value)"></label>
      <label>Lote 2<input value="${esc(lot2.lot||"")}" placeholder="Opcional" onchange="updatePhysicalExpiryField('${r.ean}',1,'lot',this.value)"></label>`:""}
    </div><small>${dbl?"Contagem final = Qtd. 1 + Qtd. 2.":"A quantidade da validade acompanha a contagem observada."}</small>
  </div>`:"";
  return `<tr id="pcrow_${r.ean}" class="${r.selected===false?"row-disabled":""}">
    ...
    <td><button type="button" class="qty-mini" onclick="changePhysicalObserved('${r.ean}',-1)" ${dbl?"disabled":""}>−</button></td>
    <td><input class="physical-manual-input" id="pcobs_${r.ean}" type="number" min="0" step="1" value="${r.observed==null?"":Math.round(r.observed)}" placeholder="Digitar" ${dbl?"readonly":""} onchange="setPhysicalObserved('${r.ean}',this.value)"></td>
    <td><button type="button" class="qty-mini" onclick="changePhysicalObserved('${r.ean}',1)" ${dbl?"disabled":""}>+</button></td>
    <td id="pcdiff_${r.ean}">${r.observed==null?"-":qtyFmt(Math.round(r.observed)-r.predicted)}</td>
    <td><button type="button" class="${has?"":"secondary"}" onclick="togglePhysicalExpiry('${r.ean}')">${has?"Editar validade":"Registrar validade"}</button>${expiryPanel}</td>
  </tr>`
}
async function togglePhysicalExpiry(ean){
  const r=physicalCountRows.find(x=>x.ean===ean);if(!r)return;
  r.hasExpiry=!r.hasExpiry;
  if(r.hasExpiry&&!r.expiryLots?.length)r.expiryLots=[{qty:r.observed??0,expiry:"",lot:""}];
  if(!r.hasExpiry){r.doubleExpiry=false;r.expiryLots=[]}
  await persistPhysicalCountMeta(ean);renderPhysicalCountProductList()
}
async function setPhysicalDoubleExpiry(ean,on){
  const r=physicalCountRows.find(x=>x.ean===ean);if(!r)return;
  r.hasExpiry=true;r.doubleExpiry=!!on;r.expiryLots=Array.isArray(r.expiryLots)?r.expiryLots:[];
  if(!r.expiryLots[0])r.expiryLots[0]={qty:r.observed??0,expiry:"",lot:""};
  if(on&&!r.expiryLots[1])r.expiryLots[1]={qty:0,expiry:"",lot:""};
  if(!on){r.expiryLots=r.expiryLots.slice(0,1);r.expiryLots[0].qty=r.observed??r.expiryLots[0].qty??0}
  if(on)await recalcPhysicalObservedFromExpiry(r);
  await persistPhysicalCountMeta(ean);renderPhysicalCountProductList()
}
async function updatePhysicalExpiryField(ean,index,field,value){
  const r=physicalCountRows.find(x=>x.ean===ean);if(!r)return;
  r.hasExpiry=true;r.expiryLots=Array.isArray(r.expiryLots)?r.expiryLots:[];
  while(r.expiryLots.length<=index)r.expiryLots.push({qty:0,expiry:"",lot:""});
  r.expiryLots[index][field]=field==="qty"?Math.max(0,Math.round(+value||0)):String(value||"");
  if(r.doubleExpiry)await recalcPhysicalObservedFromExpiry(r);
  else if(index===0&&field==="qty"){r.observed=Math.max(0,Math.round(+value||0));await persistPhysicalObserved(ean,r.observed)}
  await persistPhysicalCountMeta(ean);renderPhysicalCountProductList()
}
async function recalcPhysicalObservedFromExpiry(r){
  if(!r?.doubleExpiry)return;
  r.observed=(r.expiryLots||[]).slice(0,2).reduce((s,l)=>s+Math.max(0,Math.round(+l.qty||0)),0);
  await persistPhysicalObserved(r.ean,r.observed)
}
async function persistPhysicalCountMeta(ean){
  const cp=await ensureActiveControlPoint(),r=physicalCountRows.find(x=>x.ean===ean),p=await prod(ean),s=await get("stock",cp.unitId+"|"+ean),id=cp.id+"|"+ean,old=await get("controlPointItems",id),pred=old?.predicted??Math.round(+s?.qty||0),cost=old?.avgCost??(+s?.avgCost||+s?.lastCost||0),obs=r?.observed==null?old?.observed??0:Math.round(+r.observed||0);
  await localPut("controlPointItems",{...(old||{}),id,controlId:cp.id,unitId:cp.unitId,ean,product:p?.name||ean,predicted:pred,observed:obs,avgCost:cost,difference:obs-pred,valueDifference:(obs-pred)*cost,hasExpiry:!!r?.hasExpiry,doubleExpiry:!!r?.doubleExpiry,expiryLots:(r?.expiryLots||[]).slice(0,2).map(l=>({qty:Math.max(0,Math.round(+l.qty||0)),expiry:l.expiry||"",lot:l.lot||""})),updatedAt:new Date().toISOString()})
}
...
async function supplierVisibleProducts(){
  const q=normalizeText($("#supplierProductSearch")?.value||"");
  const rows=(await all("products")).filter(p=>p.active!==false);
  return rows.filter(p=>!q||normalizeText([p.name,p.ean,p.supplier,p.segment,p.vmPayName,...(p.aliases||[])].filter(Boolean).join(" ")).includes(q)).sort((a,b)=>String(a.name).localeCompare(String(b.name)))
}
async function renderSupplierProductList(){
  const box=$("#supplierProductList"),summary=$("#supplierProductSummary"),sid=$("#supplierRegistryFilter")?.value;
  if(!box)return;
  ...
  const offers=(await all("supplierOffers")).filter(x=>x.supplierId===sid&&x.active!==false),offerSet=new Set(offers.map(x=>x.ean));
  supplierProductSelection.clear();
  const allProducts=(await all("products")).filter(p=>p.active!==false);
  for(const p of allProducts)if(p.supplierId===sid||normalizeText(p.supplier)===normalizeText(supplier.name)||offerSet.has(p.ean))supplierProductSelection.add(p.ean);
  const rows=await supplierVisibleProducts(),linkedVisible=rows.filter(p=>supplierProductSelection.has(p.ean)).length;
  if(summary)summary.innerHTML=`<b>${supplierProductSelection.size}</b> produto(s) atendido(s) por <b>${esc(supplier.name)}</b>`;
  box.innerHTML=`<div class="supplier-products-status"><span><b>${linkedVisible}</b> vinculados visíveis</span><span>${rows.length} produto(s) na pesquisa</span></div>
  ${rows.length?`<table><thead><tr><th>✓</th><th>Produto</th><th>EAN</th><th>Fornecedor atual</th></tr></thead><tbody>${rows.map(p=>`<tr class="${supplierProductSelection.has(p.ean)?"supplier-linked-row":""}"><td><input type="checkbox" ${supplierProductSelection.has(p.ean)?"checked":""} onchange="toggleSupplierProduct('${p.ean}',this.checked)"></td><td><b>${esc(p.name)}</b></td><td>${esc(p.ean)}</td><td>${esc(p.supplier||"—")}</td></tr>`).join("")}</tbody></table>`:'<p class="muted">Nenhum produto encontrado.</p>'}`
}
