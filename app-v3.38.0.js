...
let currentSession=null,repSelectedUnits=new Set(),productUnitSelection=new Set(),supplierProductSelection=new Set();
...
function bind(){
  ...
  document.addEventListener("click",e=>{
    ...
    for(const [panelId,toggleId] of [["weeklyUnitDropdown","weeklyUnitToggle"],["weeklySupplierDropdown","weeklySupplierToggle"],["supplierProductDropdown","supplierProductToggle"]]){const pnl=$("#"+panelId),tgl=$("#"+toggleId);if(pnl&&tgl&&!pnl.classList.contains("hidden")&&!pnl.contains(e.target)&&!tgl.contains(e.target))pnl.classList.add("hidden")}
  ...
  if($("#supplierProductToggle"))$("#supplierProductToggle").onclick=()=>$("#supplierProductDropdown")?.classList.toggle("hidden");
  ...
  if($("#productUnitToggle"))$("#productUnitToggle").onclick=()=>$("#productUnitDropdown")?.classList.toggle("hidden");
  ...
  if($("#weeklyPurchaseSearch"))$("#weeklyPurchaseSearch").oninput=renderWeeklyPurchasePlan;
  if($("#weeklyUnitToggle"))$("#weeklyUnitToggle").onclick=()=>$("#weeklyUnitDropdown")?.classList.toggle("hidden");
  if($("#weeklySupplierToggle"))$("#weeklySupplierToggle").onclick=()=>$("#weeklySupplierDropdown")?.classList.toggle("hidden");
  if($("#weeklyUnitSearch"))$("#weeklyUnitSearch").oninput=renderWeeklyFilterChecks;
  if($("#weeklySupplierSearch"))$("#weeklySupplierSearch").oninput=renderWeeklyFilterChecks;
  if($("#weeklyUnitAll"))$("#weeklyUnitAll").onclick=()=>{weeklyUnitSelection=new Set(weeklyAvailableUnits.map(x=>x.id));renderWeeklyFilterChecks();renderWeeklyPurchasePlan()};
  if($("#weeklyUnitNone"))$("#weeklyUnitNone").onclick=()=>{weeklyUnitSelection.clear();renderWeeklyFilterChecks();renderWeeklyPurchasePlan()};
  if($("#weeklySupplierAll"))$("#weeklySupplierAll").onclick=()=>{weeklySupplierSelection=new Set(weeklyAvailableSuppliers);renderWeeklyFilterChecks();renderWeeklyPurchasePlan()};
  if($("#weeklySupplierNone"))$("#weeklySupplierNone").onclick=()=>{weeklySupplierSelection.clear();renderWeeklyFilterChecks();renderWeeklyPurchasePlan()};
  ...
  if($("#planogramModeProduct"))$("#planogramModeProduct").onclick=()=>setPlanogramMode("product");
  ...
  if($("#planogramProductSearch"))$("#planogramProductSearch").oninput=renderPlanogramByProduct;
  if($("#planogramProductSelect"))$("#planogramProductSelect").onchange=loadPlanogramProductUnits;
  if($("#planogramProductSelectAll"))$("#planogramProductSelectAll").onclick=async()=>{planogramProductUnits=new Set((await all("units")).filter(u=>u.active!==false).map(u=>u.id));await renderPlanogramUnitChecks()};
  if($("#planogramProductClearAll"))$("#planogramProductClearAll").onclick=async()=>{planogramProductUnits.clear();await renderPlanogramUnitChecks()};
  if($("#planogramProductSave"))$("#planogramProductSave").onclick=savePlanogramByProduct;
...
async function showApp(){
  ...
  reconcileAllProductUnitRelations().catch(e=>console.warn("Reconciliação Produto × Condomínio",e));
  // Core sync is background; UI/navigation must remain responsive.
  syncScopeWithTimeout("CORE",10000).then(async changed=>{
    if(changed){
      try{await reconcileAllProductUnitRelations();await selectors();await view(firstAllowedView())}catch(e){console.warn("Atualização Core",e)}
    }
    updateSyncState()
...
function renderCdMoveProductList(){
  ...
  $("#cdMoveProductList").innerHTML=rows.length?`<div class="cd-list-meta"><b>${qtyFmt(rows.length)}</b> produto(s) visível(is) • ${qtyFmt(cdMoveSelected.size)} selecionado(s)</div>${rows.map(x=>`<label class="check-row"><input type="checkbox" value="${x.ean}" ${cdMoveSelected.has(x.ean)?"checked":""} onchange="toggleCdMoveProduct('${x.ean}',this.checked)"><span><b>${esc(x.product)}</b><small>EAN ${x.ean} • Estoque CD ${qtyFmt(x.qty)} • ${money((+x.qty||0)*(+x.avgCost||0))}</small></span></label>`).join("")}`:'<p class="muted">Nenhum produto encontrado na pesquisa.</p>'
}
...
async function canonicalUnitProductEans(unitId,{repair=true}={}){
  ...
  const activeProducts=products.filter(p=>p.active!==false),pm=new Map(activeProducts.map(p=>[p.ean,p])),activePlan=new Map(plan.filter(pg=>pg.active!==false&&pm.has(pg.ean)).map(pg=>[pg.ean,pg])),eans=new Set(activePlan.keys());
  for(const p of activeProducts)if((p.unitIds||[]).includes(unitId))eans.add(p.ean);
  for(const st of stock)if(pm.has(st.ean)&&(+st.qty||0)>0)eans.add(st.ean);
  for(const d of demandBase)if(d.ean&&pm.has(d.ean))eans.add(d.ean);
  for(const d of demandCurrent)for(const e of(d.eans||[d.ean]))if(e&&pm.has(e))eans.add(e);
  if(repair&&unit?.type!=="CD"){
    const now=new Date().toISOString();
    for(const e of eans){
      const p=pm.get(e);if(!p)continue;
      if(!activePlan.has(e)){const idv=unitId+"|"+e,pg=plan.find(x=>x.id===idv);await localPut("planograms",{...(pg||{}),id:idv,unitId,ean:e,product:p.name,active:true,source:"RECONCILIACAO_AUTOMATICA",updatedAt:now});activePlan.set(e,true)}
...
function renderPhysicalCountProductList(){
  ...
  box.innerHTML=`<div class="physical-list-actions"><button type="button" class="secondary" onclick="setAllPhysicalSelection(true)">Selecionar visíveis</button><button type="button" class="secondary" onclick="setAllPhysicalSelection(false)">Desmarcar visíveis</button><span><b>${qtyFmt(rows.length)}</b> produtos visíveis • <b>${qtyFmt(physicalCountRows.length)}</b> cadastrados neste condomínio</span></div>
  ${rows.length?`<div class="dtable"><table><thead><tr><th>✓</th><th>Produto</th><th>EAN</th><th>Planograma</th><th>Previsto</th><th>−</th><th>Observado / digitação manual</th><th>+</th><th>Diferença</th></tr></thead><tbody>${rows.map(r=>`<tr id="pcrow_${r.ean}" class="${r.selected===false?"row-disabled":""}">
    <td><input type="checkbox" ${r.selected===false?"":"checked"} onchange="togglePhysicalSelection('${r.ean}',this.checked)"></td>
    <td><b>${esc(r.product)}</b>${r.status?`<small>${esc(r.status)}</small>`:""}</td>
    <td>${r.ean}</td><td>${r.inPlanogram?'<span class="ok">Sim</span>':'<span class="muted">Não</span>'}</td><td>${qtyFmt(r.predicted)}</td>
    <td><button type="button" class="qty-mini" onclick="changePhysicalObserved('${r.ean}',-1)">−</button></td>
    <td><input class="physical-manual-input" id="pcobs_${r.ean}" type="number" min="0" step="1" value="${r.observed==null?"":Math.round(r.observed)}" placeholder="Digitar" onchange="setPhysicalObserved('${r.ean}',this.value)"></td>
    <td><button type="button" class="qty-mini" onclick="changePhysicalObserved('${r.ean}',1)">+</button></td>
    <td id="pcdiff_${r.ean}">${r.observed==null?"-":qtyFmt(Math.round(r.observed)-r.predicted)}</td>
  </tr>`).join("")}</tbody></table></div>`:'<p class="muted">Nenhum produto encontrado na pesquisa.</p>'}`
}
...
let productsPanelMode="master",planogramMode="unit",planogramSelection=new Set(),planogramRows=[],planogramProductUnits=new Set(),planogramImportMatches=[];
async function setProductsPanel(mode){
  productsPanelMode=mode==="planogram"?"planogram":"master";
  ...
  $("#productsTabMaster").classList.toggle("active",productsPanelMode==="master");
  $("#productsTabPlanogram").classList.toggle("active",productsPanelMode==="planogram");
  if(productsPanelMode==="planogram")await renderPlanogram()
}
async function setPlanogramMode(mode){
  planogramMode=["unit","product","import"].includes(mode)?mode:"unit";
  $("#planogramByUnitPanel").classList.toggle("hidden",planogramMode!=="unit");
  $("#planogramByProductPanel").classList.toggle("hidden",planogramMode!=="product");
  ...
  $("#planogramModeProduct").classList.toggle("active",planogramMode==="product");
  $("#planogramModeImport").classList.toggle("active",planogramMode==="import");
  if(planogramMode==="unit")await renderPlanogram();
  if(planogramMode==="product")await renderPlanogramByProduct();
  if(planogramMode==="import")await renderPlanogramImport()
}
...
async function renderPlanogram(){
  ...
  planogramSelection=await planogramEans(sel.value);
...
function renderPlanogramList(){
  ...
  box.innerHTML=rows.length?rows.map(p=>`<label class="planogram-row"><input type="checkbox" ${planogramSelection.has(p.ean)?"checked":""} onchange="togglePlanogramProduct('${p.ean}',this.checked)"><span><b>${esc(p.name)}</b><small>EAN ${p.ean}${p.supplier?" • "+esc(p.supplier):""}${p.segment?" • "+esc(p.segment):""}</small></span></label>`).join(""):'<p class="muted">Nenhum produto encontrado.</p>'
}
...
async function savePlanogram(){
  ...
  const now=new Date().toISOString(),old=await byIndex("planograms","unitId",unitId);
  for(const row of old)if(!planogramSelection.has(row.ean))await localPut("planograms",{...row,active:false,updatedAt:now});
  const pm=new Map((await all("products")).map(p=>[p.ean,p]));
  for(const ean of planogramSelection){const p=pm.get(ean),id=unitId+"|"+ean,existing=await get("planograms",id);await localPut("planograms",{...(existing||{}),id,unitId,ean,product:p?.name||ean,active:true,updatedAt:now})}
  for(const row of old)await syncProductUnitIdsFromPlanogram(row.ean);for(const ean of planogramSelection)await syncProductUnitIdsFromPlanogram(ean);
...
async function renderPlanogramByProduct(){
  const products=(await all("products")).filter(p=>p.active!==false).sort((a,b)=>a.name.localeCompare(b.name)),q=normalizeText($("#planogramProductSearch")?.value||""),
        filtered=products.filter(p=>!q||normalizeText([p.name,p.ean,p.vmPayName,...(p.aliases||[]),...(p.allNames||[]),p.supplier,p.segment].filter(Boolean).join(" ")).includes(q)),
        sel=$("#planogramProductSelect"),current=sel?.value||filtered[0]?.ean||"";
  if(sel){sel.innerHTML=filtered.slice(0,1000).map(p=>`<option value="${p.ean}">${esc(p.name)} • ${p.ean}</option>`).join("");if(current&&filtered.some(p=>p.ean===current))sel.value=current}
  await loadPlanogramProductUnits()
}
async function renderPlanogramUnitChecks(){
  const units=(await all("units")).filter(u=>u.active!==false).sort((a,b)=>a.name.localeCompare(b.name));
  $("#planogramUnitChecklist").innerHTML=units.map(u=>`<label class="planogram-row"><input type="checkbox" ${planogramProductUnits.has(u.id)?"checked":""} onchange="togglePlanogramUnitForProduct('${u.id}',this.checked)"><span><b>${esc(u.name)}</b><small>${u.type==="CD"?"CD":"Condomínio / Mercado"}</small></span></label>`).join("")
}
async function loadPlanogramProductUnits(){
  const ean=$("#planogramProductSelect")?.value;planogramProductUnits=new Set();
  if(ean){for(const r of (await all("planograms")).filter(x=>x.ean===ean&&x.active!==false))planogramProductUnits.add(r.unitId)}
  await renderPlanogramUnitChecks()
}
function togglePlanogramUnitForProduct(unitId,on){on?planogramProductUnits.add(unitId):planogramProductUnits.delete(unitId)}
async function savePlanogramByProduct(){
  const ean=$("#planogramProductSelect").value,p=await prod(ean);if(!p)return alert("Selecione um produto.");
  const units=await all("units"),now=new Date().toISOString();
  for(const u of units.filter(x=>x.active!==false)){const id=u.id+"|"+ean,old=await get("planograms",id);await localPut("planograms",{...(old||{}),id,unitId:u.id,ean,product:p.name,active:planogramProductUnits.has(u.id),updatedAt:now})}
  await audit("PLANOGRAMA_PRODUTO_SALVO",{ean,units:planogramProductUnits.size});$("#planogramProductMsg").textContent=`${p.name}: ${planogramProductUnits.size} unidade(s).`;await loadPlanogramProductUnits()
}
...
async function renderExpiryCoverage(){
  const rows=await expiryCoverageSnapshot(),markets=rows.filter(x=>x.unit.type!=="CD"),totalSku=markets.reduce((s,x)=>s+x.skuWithout,0),totalUnits=markets.reduce((s,x)=>s+x.unitsWithout,0),inTransit=markets.reduce((s,x)=>s+x.inTransit,0);
  if($("#expiryCoverageSummary"))$("#expiryCoverageSummary").innerHTML=`<div class="metric"><span>SKUs sem validade</span><b>${qtyFmt(totalSku)}</b></div><div class="metric"><span>Unidades sem validade</span><b>${qtyFmt(totalUnits)}</b></div><div class="metric"><span>Unidades em trânsito</span><b>${qtyFmt(inTransit)}</b><small>Não contam como estoque físico</small></div>`;
  if($("#expiryCoverageByUnit"))$("#expiryCoverageByUnit").innerHTML=`<div class="dtable"><table><thead><tr><th>Condomínio / CD</th><th>SKUs em estoque</th><th>Unidades em estoque</th><th>SKUs sem validade</th><th>Unidades sem validade</th><th>Em trânsito</th></tr></thead><tbody>${rows.map(x=>`<tr><td>${esc(x.unit.name)}</td><td>${qtyFmt(x.totalSku)}</td><td>${qtyFmt(x.totalUnits)}</td><td><b>${qtyFmt(x.skuWithout)}</b></td><td><b>${qtyFmt(x.unitsWithout)}</b></td><td>${qtyFmt(x.inTransit)}</td></tr>`).join("")}</tbody></table></div>`
}
...
async function setStockMgmtMode(mode){stockMgmtViewMode=mode==="unit"?"unit":"general";$("#stockMgmtGeneralPanel")?.classList.toggle("hidden",stockMgmtViewMode==="unit");$("#stockMgmtUnitPanel")?.classList.toggle("hidden",stockMgmtViewMode!=="unit");$("#stockMgmtTabGeneral")?.classList.toggle("active",stockMgmtViewMode==="general");$("#stockMgmtTabUnit")?.classList.toggle("active",stockMgmtViewMode==="unit");if(stockMgmtViewMode==="unit"){await populateStockMgmtDeparaUnits();await renderStockManagementByUnit()}}
async function renderStockManagementByUnit(){
  ...
  const relevantEans=await canonicalUnitProductEans(unitId),plan=(await byIndex("planograms","unitId",unitId)).filter(x=>x.active!==false),planMap=new Map(plan.map(x=>[x.ean,x])),outsideEans=new Set();
...
async function loadProductUnitSelection(ean){productUnitSelection.clear();if(ean){const p=await prod(ean);for(const idv of(p?.unitIds||[]))productUnitSelection.add(idv);for(const r of(await all("planograms")).filter(x=>x.ean===ean&&x.active!==false))productUnitSelection.add(r.unitId)}await renderProductUnitChecklist()}
async function clearProductForm(){["pid","pname","psub","pean","psup","pseg","ploc","ppc","pncm","pcest","pvm","palias"].forEach(i=>$("#"+i).value="");productUnitSelection.clear();if($("#productUnitSearch"))$("#productUnitSearch").value="";await renderProductUnitChecklist()}
async function saveProduct(){
  ...
  const obj={...(old||{}),id:old?.id||("p_"+e),ean:e,name,subproduct:$("#psub").value.trim(),supplier:supplierName,supplierId:supplier?.id||"",segment:$("#pseg").value.trim(),
    ...
    source:old?.source||"CADASTRO_MANUAL",masterManaged:old?.masterManaged||false,unitIds:[...productUnitSelection],updatedAt:new Date().toISOString()};
  await localPut("products",obj);
  const now=new Date().toISOString(),units=(await all("units")).filter(x=>x.active!==false&&x.type!=="CD");
  for(const u of units){const idv=u.id+"|"+e,pg=await get("planograms",idv);await localPut("planograms",{...(pg||{}),id:idv,unitId:u.id,ean:e,product:name,active:productUnitSelection.has(u.id),source:pg?.source||"CADASTRO_PRODUTO",updatedAt:now})}
  if($("#productAssistBanner"))$("#productAssistBanner").classList.add("hidden");await audit("PRODUTO_SALVO",{ean:e,name,supplier:supplierName,condominios:[...productUnitSelection]});await clearProductForm();await selectors();await renderProducts()
}
...
async function refreshSupplierRegistryFilter(preserve=true){
  const el=$("#supplierRegistryFilter");if(!el)return;const old=preserve?el.value:"",rows=(await all("suppliers")).filter(x=>x.active!==false).sort((a,b)=>a.name.localeCompare(b.name));
...
async function saveSupplierRegistry(){
  const name=$("#supplierRegistryName")?.value.trim();if(!name)return alert("Informe o nome do fornecedor.");const editId=$("#supplierEditId")?.value||"",now=new Date().toISOString();let row=editId?await get("suppliers",editId):null;
  ...
  await audit("FORNECEDOR_SALVO",{supplierId:row.id,name,active:row.active});$("#supplierEditId").value="";$("#supplierRegistryName").value="";$("#supplierRegistryStatus").value="ACTIVE";await refreshSupplierRegistryFilter(false);await renderSupplierRegistryAdmin();await selectors()
}
async function editSupplierRegistry(idv){const s=await get("suppliers",idv);if(!s)return;$("#supplierEditId").value=s.id;$("#supplierRegistryName").value=s.name||"";$("#supplierRegistryStatus").value=s.active===false?"INACTIVE":"ACTIVE";window.scrollTo({top:0,behavior:"smooth"})}
async function renderSupplierRegistryAdmin(){
  await refreshSupplierRegistryFilter(true);const q=normalizeText($("#supplierRegistrySearch")?.value||""),suppliers=(await all("suppliers")).filter(x=>normalizeText(x.name).includes(q)).sort((a,b)=>a.name.localeCompare(b.name)),offers=await all("supplierOffers"),box=$("#supplierRegistryAdminList");if(!box)return;
  box.innerHTML=suppliers.length?suppliers.map(s=>{const n=offers.filter(o=>o.supplierId===s.id&&o.active!==false).length;return `<div class="row"><span><b>${esc(s.name)}</b><br><small>${n} produto(s) atendido(s) • ${s.active===false?"Inativo":"Ativo"}</small></span><span class="mini"><button type="button" onclick="editSupplierRegistry('${s.id}')">Editar</button><button type="button" class="secondary" onclick="openSupplierProducts('${s.id}')">Ver produtos</button></span></div>`}).join(""):'<p class="muted">Nenhum fornecedor cadastrado.</p>';
  if($("#supplierRegistryFilter")?.value)await loadSupplierProductSelection()
}
async function openSupplierProducts(idv){setRegistryTab("suppliers");await refreshSupplierRegistryFilter(false);$("#supplierRegistryFilter").value=idv;await loadSupplierProductSelection();$("#supplierProductDropdown")?.classList.remove("hidden")}
async function loadSupplierProductSelection(){
  supplierProductSelection.clear();const sid=$("#supplierRegistryFilter")?.value;if(sid)for(const o of(await all("supplierOffers")).filter(x=>x.supplierId===sid&&x.active!==false))supplierProductSelection.add(o.ean);await renderSupplierProductList()
}
async function supplierVisibleProducts(){const q=normalizeText($("#supplierProductSearch")?.value||"");return (await all("products")).filter(p=>p.active!==false&&(!q||normalizeText([p.name,p.ean,p.supplier].join(" ")).includes(q))).sort((a,b)=>a.name.localeCompare(b.name))}
async function renderSupplierProductList(){
  const box=$("#supplierProductList");if(!box)return;const sid=$("#supplierRegistryFilter")?.value,rows=await supplierVisibleProducts();if(!sid){box.innerHTML='<p class="muted">Selecione um fornecedor.</p>';if($("#supplierProductSummary"))$("#supplierProductSummary").textContent="";return}
  box.innerHTML=rows.length?`<table><thead><tr><th>✓</th><th>Produto</th><th>EAN</th><th>Fornecedor principal</th></tr></thead><tbody>${rows.map(p=>`<tr><td><input type="checkbox" ${supplierProductSelection.has(p.ean)?"checked":""} onchange="toggleSupplierProduct('${p.ean}',this.checked)"></td><td><b>${esc(p.name)}</b></td><td>${esc(p.ean)}</td><td>${esc(p.supplier||"—")}</td></tr>`).join("")}</tbody></table>`:'<p class="muted">Nenhum produto encontrado.</p>';
  if($("#supplierProductSummary"))$("#supplierProductSummary").textContent=`${supplierProductSelection.size} produto(s) selecionado(s)`
}
function toggleSupplierProduct(ean,on){on?supplierProductSelection.add(ean):supplierProductSelection.delete(ean);renderSupplierProductList()}
async function setVisibleSupplierProducts(on){for(const p of await supplierVisibleProducts())on?supplierProductSelection.add(p.ean):supplierProductSelection.delete(p.ean);await renderSupplierProductList()}
async function saveSupplierProductCoverage(){
  const sid=$("#supplierRegistryFilter")?.value;if(!sid)return alert("Selecione o fornecedor.");const supplier=await get("suppliers",sid);if(!supplier)return;const now=new Date().toISOString(),existing=(await all("supplierOffers")).filter(x=>x.supplierId===sid),existingMap=new Map(existing.map(x=>[x.ean,x]));
  for(const p of (await all("products")).filter(x=>x.active!==false)){const key=sid+"|"+p.ean,old=existingMap.get(p.ean);if(supplierProductSelection.has(p.ean))await localPut("supplierOffers",{...(old||{}),id:key,supplierId:sid,supplierName:supplier.name,ean:p.ean,lastCost:+old?.lastCost||0,lastPurchaseAt:old?.lastPurchaseAt||"",active:true,updatedAt:now});else if(old&&old.active!==false)await localPut("supplierOffers",{...old,active:false,updatedAt:now})}
  await audit("FORNECEDOR_PRODUTOS_ATUALIZADOS",{supplierId:sid,supplier:supplier.name,products:supplierProductSelection.size});await renderSupplierRegistryAdmin();await renderSupplierProductList();alert("Produtos atendidos pelo fornecedor atualizados.")
}
// ---------- Estoque / Inventário ----------
...
async function renderEntries(){
  ...
  renderPurchaseItems();renderPurchaseDistribution();if(weeklyPurchaseRows.length)await populateWeeklyPurchaseFilters();renderWeeklyPurchasePlan();if(!$("#purchaseReplenishmentPanel")?.classList.contains("hidden"))await renderReplenishment();
...
function showPurchaseTab(t){
  const tabs=["weekly","manual","nf"];if(!tabs.includes(t))t="weekly";
  $("#purchaseWeeklyPanel")?.classList.toggle("hidden",t!=="weekly");$("#purchaseReplenishmentPanel")?.classList.toggle("hidden",t!=="replenishment");$("#purchaseManualPanel")?.classList.toggle("hidden",t!=="manual");$("#purchaseNFPanel")?.classList.toggle("hidden",t!=="nf");
  $("#purchaseTabWeekly")?.classList.toggle("active",t==="weekly");$("#purchaseTabReplenishment")?.classList.toggle("active",t==="replenishment");$("#purchaseTabManual")?.classList.toggle("active",t==="manual");$("#purchaseTabNF")?.classList.toggle("active",t==="nf");
  if(t==="weekly")renderWeeklyPurchasePlan();if(t==="replenishment")renderReplenishment()
}
...
let weeklyPurchaseRows=[],weeklyPurchaseView="UNIT",weeklyPurchaseGeneratedAt="",weeklyUnitSelection=new Set(),weeklySupplierSelection=new Set(),weeklyAvailableUnits=[],weeklyAvailableSuppliers=[];
function weeklyPurchaseSelectedRows(){
  const q=normalizeText($("#weeklyPurchaseSearch")?.value||"");
  return weeklyPurchaseRows.filter(x=>(!weeklyUnitSelection.size||weeklyUnitSelection.has(x.unitId))&&(!weeklySupplierSelection.size||weeklySupplierSelection.has(x.supplier||""))&&(!q||normalizeText([x.product,x.ean,x.unit,x.supplier].join(" ")).includes(q)))
}
...
function renderWeeklyFilterChecks(){
  ...
  if(ub)ub.innerHTML=weeklyAvailableUnits.filter(u=>!uq||normalizeText(u.name).includes(uq)).map(u=>`<label><input type="checkbox" ${weeklyUnitSelection.has(u.id)?"checked":""} onchange="toggleWeeklyUnit('${u.id}',this.checked)"> ${esc(u.name)}</label>`).join("")||'<small class="muted">Nenhum condomínio encontrado.</small>';
  if(sb)sb.innerHTML=weeklyAvailableSuppliers.filter(n=>!sq||normalizeText(n).includes(sq)).map(n=>`<label><input type="checkbox" ${weeklySupplierSelection.has(n)?"checked":""} onchange="toggleWeeklySupplier('${esc(n).replace(/'/g,"&#39;")}',this.checked)"> ${esc(n)}</label>`).join("")||'<small class="muted">Nenhum fornecedor encontrado.</small>';
  if($("#weeklyUnitSelectionSummary"))$("#weeklyUnitSelectionSummary").textContent=weeklyUnitSelection.size?`${weeklyUnitSelection.size} condomínio(s) selecionado(s)`:"Todos os condomínios";
  if($("#weeklySupplierSelectionSummary"))$("#weeklySupplierSelectionSummary").textContent=weeklySupplierSelection.size?`${weeklySupplierSelection.size} fornecedor(es) selecionado(s)`:"Todos os fornecedores"
}
async function populateWeeklyPurchaseFilters(){
  ...
  weeklyAvailableSuppliers=[...new Set(weeklyPurchaseRows.map(x=>x.supplier).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  if(!weeklyUnitSelection.size)weeklyUnitSelection=new Set(weeklyAvailableUnits.map(x=>x.id));
  if(!weeklySupplierSelection.size)weeklySupplierSelection=new Set(weeklyAvailableSuppliers);
  weeklyUnitSelection=new Set([...weeklyUnitSelection].filter(id=>weeklyAvailableUnits.some(u=>u.id===id)));weeklySupplierSelection=new Set([...weeklySupplierSelection].filter(n=>weeklyAvailableSuppliers.includes(n)));
  renderWeeklyFilterChecks()
}
async function buildWeeklyPurchasePlan(){
  const days=Math.max(1,Math.round(+$("#weeklyCoverageDays")?.value||7)),ctx=await buildOperationalContext(),markets=ctx.units.filter(x=>x.active!==false&&x.type!=="CD"),cd=ctx.units.find(x=>x.active!==false&&x.type==="CD"&&x.primaryCD!==false)||ctx.units.find(x=>x.active!==false&&x.type==="CD"),[planograms,offersAll]=await Promise.all([all("planograms"),all("supplierOffers")]),planByUnit=new Map(),offerByEan=new Map();
  for(const pg of planograms.filter(x=>x.active!==false)){const s=planByUnit.get(pg.unitId)||new Set();s.add(pg.ean);planByUnit.set(pg.unitId,s)}for(const o of offersAll.filter(x=>x.active!==false)){const a=offerByEan.get(o.ean)||[];a.push(o);offerByEan.set(o.ean,a)}
  ...
  for(const u of markets){
    ...
    const plan=planByUnit.get(u.id)||new Set();
...
function renderWeeklyPurchasePlan(){
  ...
  if(weeklyPurchaseView==="PRODUCT"){const c=consolidatedWeeklyPurchaseRows(rows);box.innerHTML=c.length?`<div class="dtable weekly-purchase-table"><table><thead><tr><th>Fornecedor</th><th>Produto</th><th>EAN</th><th>Condomínios</th><th>Média semanal</th><th>Demanda período</th><th>Do CD</th><th>Compra mínima</th><th>Compra recomendada</th><th>Custo estimado</th><th>Valor estimado</th></tr></thead><tbody>${c.map(x=>`<tr class="${x.recommendedBuy>0?"weekly-buy-row":""}"><td>${esc(x.supplier||"—")}</td><td><b>${esc(x.product)}</b></td><td>${esc(x.ean)}</td><td>${x.units.size}</td><td>${qtyFmt(x.averageWeekly)}</td><td>${qtyFmt(x.demandPeriod)}</td><td>${qtyFmt(x.fromCd)}</td><td><b>${qtyFmt(x.minimumBuy)}</b></td><td><b class="warn">${qtyFmt(x.recommendedBuy)}</b></td><td>${x.estimatedCost?money(x.estimatedCost):"—"}</td><td>${x.estimatedCost?money(x.recommendedBuy*x.estimatedCost):"—"}</td></tr>`).join("")}</tbody></table></div>`:'<p class="muted">Nenhuma compra recomendada com os filtros selecionados.</p>'}else{box.innerHTML=rows.length?`<div class="dtable weekly-purchase-table"><table><thead><tr><th>Condomínio</th><th>Produto</th><th>EAN</th><th>Estoque</th><th>Em aberto</th><th>Média semanal</th><th>Demanda ${days}d</th><th>Alerta</th><th>Reposição hoje</th><th>Do CD</th><th>Compra mínima</th><th>Compra recomendada</th><th>Fornecedor</th></tr></thead><tbody>${rows.sort((a,b)=>a.unit.localeCompare(b.unit)||a.product.localeCompare(b.product)).map(x=>`<tr class="${x.immediateStatus==="OK"&&x.recommendedBuy>0?"weekly-preventive-row":x.immediateStatus==="RUPTURA"?"need-high":x.immediateStatus==="REPOSIÇÃO"?"need-mid":""}"><td>${esc(x.unit)}</td><td><b>${esc(x.product)}</b></td><td>${esc(x.ean)}</td><td>${qtyFmt(x.stock)}</td><td>${qtyFmt(x.inbound)}</td><td>${qtyFmt(x.averageWeekly)}</td><td>${qtyFmt(x.demandPeriod)}</td><td>${qtyFmt(x.alert)}</td><td><span class="tag ${x.immediateStatus==="OK"?"ok":x.immediateStatus==="RUPTURA"?"bad":"warn"}">${x.immediateStatus==="OK"?"NÃO PRECISA":esc(x.immediateStatus)}</span></td><td>${qtyFmt(x.fromCd)}</td><td><b>${qtyFmt(x.minimumBuy)}</b></td><td><b class="warn">${qtyFmt(x.recommendedBuy)}</b>${x.immediateStatus==="OK"&&x.recommendedBuy>0?'<small class="weekly-preventive-note">compra preventiva</small>':""}</td><td>${esc(x.supplier||"—")}</td></tr>`).join("")}</tbody></table></div>`:'<p class="muted">Nenhuma compra recomendada com os filtros selecionados.</p>'}
}
...
async function saveExpiry(){
  ...
  if(!p){await ensureProductOrOfferRegistration(e,{ean:e});return}
  if(!date)return alert("Informe validade.");
  if(!u)return alert("Selecione a unidade.");
  if(q<=0)return alert("Informe a quantidade.");
  const stock=await get("stock",u+"|"+e),available=Math.max(0,Math.round(+stock?.qty||0)),pending=await pendingInboundExpiryQty(u,e);
  if(available<=0){
    if(pending>0)return alert(`O estoque físico desta unidade está zerado. Existem ${qtyFmt(pending)} unidade(s) previstas/em trânsito. A validade deve permanecer vinculada à movimentação até o recebimento físico.`);
    if(confirm(`O sistema registra estoque 0 para ${p.name}. Se o produto está fisicamente no local, corrija primeiro pela Contagem de Estoque Física. Deseja abrir Contagem de Estoque?`))view("controlstock");
    return
  }
  if(q>available)return alert(`Quantidade informada excede o estoque físico atual (${qtyFmt(available)}).`);
  // Se o estoque físico veio de versão antiga/contagem sem metadados de lote, crie somente a classificação SEM VALIDADE, sem alterar o estoque.
  const beforeLots=(await byIndex("lots","ean",e)).filter(x=>x.unitId===u),beforeLotQty=Math.round(beforeLots.reduce((s,x)=>s+(+x.qty||0),0));
  if(beforeLotQty<available)await reconcileLotsToStock(u,e,available);
  const current=(await byIndex("lots","ean",e)).filter(x=>x.unitId===u),unknown=current.filter(x=>!x.expiry&&+x.qty>0).sort((a,b)=>String(a.updatedAt||"").localeCompare(String(b.updatedAt||""))),unknownQty=Math.round(unknown.reduce((s,x)=>s+(+x.qty||0),0));
  if(q>unknownQty)return alert(`Há apenas ${qtyFmt(unknownQty)} unidade(s) físicas sem validade conhecida para classificar. O módulo de Validades não cria estoque.`);
  let left=q;
  for(const r of unknown){if(left<=0)break;const take=Math.min(left,Math.round(+r.qty||0));r.qty-=take;r.updatedAt=new Date().toISOString();await localPut("lots",r);left-=take}
  await upsertLot(u,e,q,date,lot,+(stock?.avgCost||0),"VALIDADE_MANUAL");
  await localPut("expiries",{id:id("e"),unitId:u,ean:e,product:p.name,date,qty:q,lot,photo:await imageFileData($("#ephoto")?.files?.[0]),productPhoto:await imageFileData($("#eproductphoto")?.files?.[0]),stockAtRegistration:available,updatedAt:new Date().toISOString()});
  await audit("VALIDADE_REGISTRADA",{unitId:u,ean:e,qty:q,date,lot,stockAtRegistration:available});
  await renderExpiry()
}
...
async function loadMaster(){
  ...
  try{
    for(const store of ["products","units","planograms"]){const r=await apiGet("list",{store});if(Array.isArray(r.rows)){await clearStore(store);for(const row of r.rows)await put(store,row)}}
...
async function addControlItem(){
  if(!activeControlId)return alert("Inicie uma contagem de estoque física primeiro.");
...
async function approveControlPoint(){
  await captureStockSnapshotBefore("ANTES_APROVAR_CONTAGEM_FISICA",{source:"approveControlPoint"});
