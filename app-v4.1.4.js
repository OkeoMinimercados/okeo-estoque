...
const EXPECTED_BACKEND_VERSION="4.1.4";
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=4.1.4",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="4.1.4"){
        localStorage.setItem("okeo_frontend_cache_version","4.1.4")
      }
    })
...
async function verifyBackendVersion(expected="4.1.4"){
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("4.1.4");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 4.1.4.`;
      alert(`Publique primeiro o backend 4.1.4. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    ...
    const resumable=progress&&progress.version==="4.1.4";
    if(!resumable){
      ...
      await backup();
      progress={version:"4.1.4",storeIndex:0,rowIndex:0,sent:0,startedAt:new Date().toISOString()};
...
async function runDeepHealthCheck(){
  ...
  if(remote.ok){
    ...
    add("Backend",status.version==="4.1.4",`publicado v${status.version||"?"} • esperado v4.1.4`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="4.1.4",`Versão ${status.version||"?"}`);
