...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=3.77.0",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="3.77.0"){
        localStorage.setItem("okeo_frontend_cache_version","3.77.0")
      }
    })
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.77.json",{cache:"no-store"});
