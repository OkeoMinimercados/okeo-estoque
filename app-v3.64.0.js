...
function bind(){
  ...
  if($("#resetGeneralOpen"))$("#resetGeneralOpen").onclick=openGeneralReset;if($("#loadMaster"))$("#loadMaster").onclick=loadMaster;$("#loadDemandSnapshot").onclick=loadDemandSnapshot;
  if($("#demandSnapshotFile"))$("#demandSnapshotFile").onchange=async()=>{const n=$("#demandSnapshotFile").files?.length||0;$("#demandSnapshotMsg").textContent=n?`${n} arquivo(s) selecionado(s). Clique em Importar e calcular demanda.`:"";await markDemandPending(n?`${n} arquivo(s) selecionado(s)`:"Seleção de arquivos alterada");await renderDemandUpdateControl()};
  if($("#demandRefreshStatus"))$("#demandRefreshStatus").onclick=renderDemandUpdateControl;
  if($("#demandRecalculateNow"))$("#demandRecalculateNow").onclick=calcDemand;
...
const OKEO_FILTER_CHANGE_ROUTES={
  ...
  dunit:async()=>{if($("#demandCalcMsg"))$("#demandCalcMsg").textContent="Visão alterada. Clique em Calcular / atualizar demanda.";await markDemandPending("Visão alterada")},
  dweeks:async()=>{if($("#demandCalcMsg"))$("#demandCalcMsg").textContent="Período alterado. Clique em Calcular / atualizar demanda.";await markDemandPending("Período alterado")}
};
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=3.64.0",{updateViaCache:"none"});
    await reg.update();
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.64.json",{cache:"no-store"});
...
async function renderDemandUpdateControl(){
  ...
  const [state,base,current,metaRow]=await Promise.all([
    getDemandUpdateState(),all("demandBase"),all("demandCurrent"),get("settings","DEMAND_PROCESSING_META")
  ]);
  const meta=metaRow?.value||{},files=$("#demandSnapshotFile")?.files?.length||0,currentWeeks=Math.max(1,Math.round(+$("#dweeks")?.value||12)),
        currentView=$("#dunit")?.value||"TOTAL_CONSOLIDADO",
        calculatedWeeks=+state.lastCalculatedWeeks||0,
        isPeriodDirty=!!calculatedWeeks&&calculatedWeeks!==currentWeeks,
        selectedFilesDirty=files>0,
        effectiveStatus=state.status==="PROCESSING"?"PROCESSING":state.status==="ERROR"?"ERROR":(!base.length?"EMPTY":(selectedFilesDirty||isPeriodDirty||state.status==="PENDING"?"PENDING":"UPDATED")),
        [label,cls]=demandStatusInfo(effectiveStatus),
        lastImport=state.lastImportAt||meta.processedAt||"",
        lastCalc=state.lastCalculationAt||current.map(x=>x.calculatedAt).filter(Boolean).sort().pop()||"";
  box.innerHTML=`<div class="demand-status-strip ${cls}">
    <div><span>Status</span><b>${label}</b>${state.currentStep?`<small>${esc(state.currentStep)}</small>`:""}</div>
    <div><span>Arquivos selecionados</span><b>${files}</b><small>${files?"Aguardando importação/reprocessamento":"Nenhum novo arquivo"}</small></div>
    <div><span>Base histórica</span><b>${qtyFmt(base.length)}</b><small>${meta.periodStart&&meta.periodEnd?`${esc(meta.periodStart)} a ${esc(meta.periodEnd)}`:"Sem período registrado"}</small></div>
    <div><span>Demanda operacional</span><b>${qtyFmt(current.length)}</b><small>${current.length?"Registros calculados":"Ainda não calculada"}</small></div>
  </div>
  <div class="demand-control-details">
    ...
    <div><span>Último cálculo</span><b>${lastCalc?new Date(lastCalc).toLocaleString("pt-BR"):"—"}</b></div>
    <div><span>Período do último cálculo</span><b>${calculatedWeeks?calculatedWeeks+" semanas":"—"}</b></div>
    <div><span>Período selecionado agora</span><b>${currentWeeks} semanas</b></div>
    <div><span>Visão calculada</span><b>${esc(state.lastCalculatedView||"—")}</b></div>
    <div><span>Última duração</span><b>${state.lastDurationSeconds!=null?n2(state.lastDurationSeconds)+" s":"—"}</b></div>
  </div>
  ${isPeriodDirty?'<div class="demand-control-warning">O período foi alterado depois do último cálculo. Clique em <b>Recalcular agora</b>.</div>':""}
  ${selectedFilesDirty?'<div class="demand-control-warning">Existem arquivos selecionados que ainda não foram incorporados à Base Histórica. Use <b>Importar e calcular demanda</b>.</div>':""}
