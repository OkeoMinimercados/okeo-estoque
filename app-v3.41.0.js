...
async function showApp(){
  ...
  Promise.resolve().then(async()=>{
    try{
      await ensureReferenceDataAvailable("LOGIN_V341");
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.41.json",{cache:"no-store"});
...
async function canonicalUnitProductEans(unitId,{repair=true}={}){
  ...
  const activeProducts=products.filter(p=>p.active!==false),pm=new Map(activeProducts.map(p=>[p.ean,p])),activePlan=new Map(plan.filter(pg=>pg.active!==false&&pm.has(pg.ean)).map(pg=>[pg.ean,pg])),explicitOff=new Set(plan.filter(pg=>pg.active===false&&pm.has(pg.ean)).map(pg=>pg.ean)),eans=new Set(activePlan.keys()),repairable=new Set(activePlan.keys());
  if(unit?.type==="CD")return new Set(activeProducts.map(p=>p.ean));
  // Um vínculo explicitamente desmarcado no Planograma prevalece sobre fontes auxiliares/legadas.
  ...
  // V3.41: relação histórica real Produto × Condomínio, derivada das bases de planograma já homologadas.
  for(const e of await seedUnitProductEans(unit,activeProducts))if(!explicitOff.has(e))eans.add(e);
  // Repara somente vínculos comprovados pelas bases operacionais locais/centrais. O seed é fallback de leitura e só é persistido quando o usuário salva/usa o item.
...
async function renderPlanogramUnitChecks(){
  const units=(await all("units")).filter(u=>u.active!==false&&u.type!=="CD").sort((a,b)=>a.name.localeCompare(b.name));
  $("#planogramUnitChecklist").innerHTML=units.length?`<div class="dtable planogram-unit-table"><table><thead><tr><th>✓</th><th>Condomínio</th></tr></thead><tbody>${units.map(u=>`<tr><td><input type="checkbox" ${planogramProductUnits.has(u.id)?"checked":""} onchange="togglePlanogramUnitForProduct('${u.id}',this.checked)"></td><td><b>${esc(u.name)}</b></td></tr>`).join("")}</tbody></table></div>`:'<p class="muted">Nenhum condomínio ativo cadastrado.</p>'
}
async function loadPlanogramProductUnits(){
  const ean=$("#planogramProductSelect")?.value;planogramProductUnits=new Set();
  if(ean){const units=(await all("units")).filter(u=>u.active!==false&&u.type!=="CD");for(const u of units){const rel=await canonicalUnitProductEans(u.id,{repair:false});if(rel.has(ean))planogramProductUnits.add(u.id)}}
...
async function savePlanogramByProduct(){
  ...
  const units=(await all("units")).filter(x=>x.active!==false&&x.type!=="CD"),now=new Date().toISOString();
  for(const u of units){const id=u.id+"|"+ean,old=await get("planograms",id);await localPut("planograms",{...(old||{}),id,unitId:u.id,ean,product:p.name,active:planogramProductUnits.has(u.id),updatedAt:now})}
  await syncProductUnitIdsFromPlanogram(ean);await audit("PLANOGRAMA_PRODUTO_SALVO",{ean,units:planogramProductUnits.size});$("#planogramProductMsg").textContent=`${p.name}: ${planogramProductUnits.size} unidade(s).`;await loadPlanogramProductUnits()
}
