...
const EXPECTED_BACKEND_VERSION="4.1.8";
...
async function resolveBackendUrl(){
  let local=String(localStorage.getItem(BACKEND_STORAGE_KEY)||"").trim(),dbUrl="";
  try{dbUrl=String((await get("settings","backend"))?.url||"").trim()}catch(e){}
  // A URL explicitamente salva neste dispositivo é a autoridade. O default só
  // entra quando nenhuma conexão foi configurada.
  const url=local||dbUrl||DEFAULT_BACKEND_URL;
  localStorage.setItem(BACKEND_STORAGE_KEY,url);
  if(local!==url||!local)localStorage.setItem("okeo_backend_custom_allowed","1");
  try{await put("settings",{id:"backend",url,customAllowed:true})}catch(e){}
...
async function setBackendUrl(url){
  ...
  if(clean&&!/^https:\/\/script\.google\.com\/macros\/s\/[^/]+\/exec(?:\?.*)?$/i.test(clean)){
    throw new Error("Use uma URL /exec publicada pelo Google Apps Script.")
  }
  // Valida a URL digitada DIRETAMENTE. Não consulta resolveBackendUrl e não altera
  // a conexão ativa antes de saber exatamente qual backend está respondendo.
  const probe=await probeBackendUrlDirect(clean,12000);
  const version=String(probe.version||"");
  if(version!==EXPECTED_BACKEND_VERSION){
    throw new Error(`URL não ativada: esta implantação responde backend ${version||"indisponível"}; esperado ${EXPECTED_BACKEND_VERSION}. A conexão anterior foi preservada.`)
  }
  const isOfficial=clean===DEFAULT_BACKEND_URL;
  if(isOfficial)localStorage.removeItem("okeo_backend_custom_allowed");
  else localStorage.setItem("okeo_backend_custom_allowed","1");
  const previousUrl=getBackendUrl();
  localStorage.setItem(BACKEND_STORAGE_KEY,clean);
  try{await put("settings",{id:"backend",url:clean,customAllowed:!isOfficial,verifiedVersion:version,verifiedAt:new Date().toISOString(),deploymentTail:backendDeploymentTail(clean)})}catch(e){}
  if(previousUrl&&previousUrl!==clean){
    // Token/sessão pertencem ao backend que os emitiu. Nunca reaproveitar após troca de Central.
    localStorage.removeItem("okeo_session");
    sessionStorage.removeItem("okeo_session");
    currentSession=null;
    localStorage.setItem("okeo_backend_switched_at",new Date().toISOString())
  }
  invalidateBackendConnectionCache();
  backendCompatibility={checkedAt:Date.now(),version,ok:true,productionMode:!!probe.productionMode,snapshotEpoch:String(probe.snapshotEpoch||"")};
  const localEpoch=String(localStorage.getItem("okeo_device_snapshot_epoch")||"");
  if(!probe.productionMode||!probe.snapshotEpoch||localEpoch!==String(probe.snapshotEpoch||"")){
...
async function startupPreflight(){
  ...
  try{
    ...
    if(!c.ok)issues.push(`Versões incompatíveis: frontend ${FRONTEND_VERSION} × backend ${c.version||"indisponível"}. Escritas na Base Central estão bloqueadas.`);
    const errors=getCurrentRuntimeErrors();
    if(errors.length)issues.push(`${errors.length} erro(s) operacional(is) atual(is) registrado(s).`);
    const q=await countStore("syncQueue");
    if(q>500)issues.push(`Fila de sincronização elevada: ${q} operações pendentes.`);
    const products=await countStore("products"),
          localEpoch=String(localStorage.getItem("okeo_device_snapshot_epoch")||"");
    if(c.productionMode&&products===0)issues.push("Este dispositivo está sem Produtos apesar de a Base Central estar em produção.");
    if(c.productionMode&&c.snapshotEpoch&&localEpoch!==c.snapshotEpoch){
      ...
      issues.push(`Snapshot deste dispositivo está desatualizado (${localEpoch||"nenhum"} → ${c.snapshotEpoch.slice(0,8)}). A fila local está bloqueada e NÃO será enviada. Atualize pela Base Central antes de operar.`);
    }else if(c.ok){
      clearDeviceHydrationRequired()
    }
    if(issues.length){
      const hydrateButton=isDeviceHydrationRequired()?` <button type="button" onclick="downloadCentralBaseToDevice()">Atualizar dispositivo agora</button>`:` <button type="button" onclick="openSystemHealth()">Abrir Saúde</button>`;
      banner.innerHTML=`<b>Pré-checagem:</b> ${issues.map(esc).join(" • ")}${hydrateButton}`;
      banner.classList.remove("hidden")
    }else{
      banner.textContent="Pré-checagem OK • frontend/backend compatíveis • dados prontos.";
...
async function showApp(){
  ...
  applyAccessProfile();
  // Dashboard abre imediatamente; qualquer mutação em segundo plano espera a pré-checagem.
  await view(firstAllowedView());
  const preflight=await startupPreflight().catch(e=>({ok:false,issues:[String(e?.message||e)]}));
  if(isDeviceHydrationRequired()){
    updateSyncState("Atualização Central necessária","warn");
    // Dispositivo desatualizado fica em modo leitura até receber o snapshot Central.
    return
  }
  ...
  setTimeout(()=>hydrateNewDeviceFromCentral().catch(()=>{}),1600);
  setTimeout(()=>runPostInitRegressionAudit().catch(()=>{}),2200)
}
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=4.1.8",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="4.1.8"){
        localStorage.setItem("okeo_frontend_cache_version","4.1.8")
      }
    })
    ...
    syncScopeWithTimeout(scope,8000).then(async changed=>{
      if(changed&&$("#"+v)&&!$("#"+v).classList.contains("hidden")){
        try{if(fn)await fn()}
          ...
          console.warn("Re-render",v,e);
          rememberRuntimeError("RERENDER_"+v,e)
        }
      }
    }).catch(e=>{
      console.warn("Sync em segundo plano",v,e);
      rememberRuntimeError("SYNC_"+scope,e)
    })
  }
}
...
function assertDeviceOperationalWriteAllowed(store){
  if(SYNC_STORES.includes(store)&&isDeviceHydrationRequired()){
    const e=new Error("DEVICE_REQUIRES_CENTRAL_UPDATE");
    e.code="DEVICE_REQUIRES_CENTRAL_UPDATE";
    throw e
  }
}
...
function currentDeviceEpoch(){return String(localStorage.getItem("okeo_device_snapshot_epoch")||"")}
...
async function refreshDeviceProductionState(force=false){
  ...
  if(c.ok&&c.productionMode&&c.snapshotEpoch&&localEpoch!==c.snapshotEpoch){
    markDeviceHydrationRequired(c.snapshotEpoch,"SNAPSHOT_OUTDATED");
    return {ok:false,blocked:true,compat:c,reason:"SNAPSHOT_OUTDATED"}
  }
  if(c.ok&&(!c.productionMode||!c.snapshotEpoch||localEpoch===c.snapshotEpoch)){
    clearDeviceHydrationRequired()
  }
  return {ok:c.ok,blocked:false,compat:c}
}
async function processQueue(){
  ...
  const deviceState=await refreshDeviceProductionState(false);
  if(deviceState.blocked||isDeviceHydrationRequired()){
    setSyncState(`${q.length} antiga(s) bloqueada(s)`,"warn");
    return {blocked:true,pending:q.length,reason:"SNAPSHOT_OUTDATED"}
  }
...
async function verifyBackendVersion(expected="4.1.8"){
...
async function prepareProductionBase(){
  ...
  try{
    ...
    localStorage.setItem("okeo_device_snapshot_epoch",String(fin.epoch||""));
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("4.1.8");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 4.1.8.`;
      alert(`Publique primeiro o backend 4.1.8. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    ...
    const resumable=progress&&progress.version==="4.1.8";
    if(!resumable){
      ...
      await backup();
      progress={version:"4.1.8",storeIndex:0,rowIndex:0,sent:0,startedAt:new Date().toISOString()};
...
async function runDeepHealthCheck(){
  ...
  const deviceEpoch=String(localStorage.getItem("okeo_device_snapshot_epoch")||"");
  if(remote.ok){
    const status=remote.status||{};
    add("Base Central online",true,`resposta ${remote.ms} ms • implantação ${remote.deploymentTail||"?"}`);
    add("Backend",status.version==="4.1.8",`publicado v${status.version||"?"} • esperado v4.1.8`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="4.1.8",`Versão ${status.version||"?"}`);
...
async function downloadCentralBaseToDevice(){
  ...
  try{
    ...
    const previous={},oldQueue=await all("syncQueue");
    ...
    try{
      ...
      localStorage.setItem("okeo_device_snapshot_epoch",compat.snapshotEpoch);
      ...
      try{await clearStore("syncQueue");if(oldQueue.length)await putMany("syncQueue",oldQueue)}catch(e){console.error("ROLLBACK_QUEUE",e)}
      throw applyError
    }
  }catch(e){
...
async function hydrateNewDeviceFromCentral(){
  if(!currentSession?.token||localStorage.getItem("okeo_full_central_download_at"))return;
  try{
    const state=await refreshDeviceProductionState(false);
    if(!state.compat?.ok||!state.compat.productionMode||!state.compat.snapshotEpoch)return;
    const [localProducts,pending]=await Promise.all([countStore("products"),countStore("syncQueue")]);
    // Só hidrata automaticamente um dispositivo realmente vazio e sem fila.
    if(localProducts===0&&pending===0){
      await downloadCentralBaseToDevice()
    }
  }catch(e){console.warn("Hidratação automática do dispositivo",e)}
}
// ---------- settings / backup ----------
async function saveBackend(){
  const u=$("#backend").value.trim(),m=$("#syncMsg");
  m.textContent="Validando diretamente a nova URL...";
  try{
    const r=await setBackendUrl(u),s=r.status;
    m.textContent=`URL ativada • backend ${s.version} • implantação ${backendDeploymentTail(r.url)||"?"} • produção ${s.productionMode?"ativa":"inativa"} • snapshot ${String(s.snapshotEpoch||"nenhum").slice(0,8)}`;
    await refreshLoginConnectionStatus(true);await updateSyncState();
    if(!currentSession){
      showLogin();
      $("#loginMsg").textContent="Base Central alterada. Faça login novamente nesta Central.";
      return
    }
    await startupPreflight()
...
async function renderSettings(){
  const s=await get("settings","backend");
  $("#backend").value=getBackendUrl()||s?.url||DEFAULT_BACKEND_URL;
