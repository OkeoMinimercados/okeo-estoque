...
const EXPECTED_BACKEND_VERSION="4.1.0";
...
async function registerServiceWorkerSafely(){
  ...
  try{
    ...
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="4.1.0"){
        localStorage.setItem("okeo_frontend_cache_version","4.1.0")
      }
    })
...
async function verifyBackendVersion(expected="4.1.0"){
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("4.1.0");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 4.1.0.`;
      alert(`Publique primeiro o backend 4.1.0. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    ...
    const resumable=progress&&progress.version==="4.1.0";
    if(!resumable){
      ...
      await backup();
      progress={version:"4.1.0",storeIndex:0,rowIndex:0,sent:0,startedAt:new Date().toISOString()};
...
async function runDeepHealthCheck(){
  ...
  try{
    ...
    add("Correção Local → Central",!!$("#healthRepairSync"),$("#healthRepairSync")?"botão presente":"ausente");
    ...
    const status=await apiGet("status",{}, {timeoutMs:7000});
    add("Backend",status.version==="4.1.0",`publicado v${status.version||"?"} • esperado v4.1.0`);
    ...
    const central=await getCentralStoreCounts(),local=await getLocalStoreCounts();
    const important=["products","units","suppliers","supplierOffers","planograms","stock","purchases","controlPoints"];
    for(const s of important)add("Sync "+s,(+local[s]||0)===(+central[s]||0),`Local ${+local[s]||0} × Central ${+central[s]||0}`);

    const dataIntegrity=await runOperationalDataIntegrity();
    add("Integridade operacional",dataIntegrity.ok,dataIntegrity.ok?"Referências consistentes":`${dataIntegrity.issues.length} pendência(s)`);
    const recent=getRecentRuntimeErrors();
    add("Erros recentes do navegador",recent.length===0,recent.length?`${recent.length} erro(s) registrado(s)`:"Nenhum");
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="4.1.0",`Versão ${status.version||"?"}`);
    ...
    const runtimeErrors=getRecentRuntimeErrors();
    add("Erros JavaScript recentes",runtimeErrors.length===0,runtimeErrors.length?`${runtimeErrors.length} registrado(s)`:"Nenhum");
