...
function bind(){
  ...
  if($("#repDraft"))if($("#repDraft"))$("#repDraft").onclick=generateReplenishmentDraft;if($("#repApprove"))if($("#repApprove"))$("#repApprove").onclick=approveReplenishment;if($("#cstart"))$("#cstart").onclick=startControlPoint;if($("#capprove"))$("#capprove").onclick=approveControlPoint;if($("#cadd"))$("#cadd").onclick=addControlItem;
...
// ---------- FILTROS ROBUSTOS V3.50 ----------
function installRobustFilterBindings(){
  const inputMap={
    psearch:()=>renderProducts(),planogramSearch:()=>renderPlanogramProducts(),productsBySupplierSearch:()=>renderProductsBySupplier(),
    supplierDraftProductSearch:()=>renderSupplierDraftProductList(),supplierProductSearch:()=>renderSupplierProductList(),supplierRegistrySearch:()=>renderSupplierRegistryAdmin(),
    physicalCountSearch:()=>renderPhysicalCountProductList(),weeklyUnitSearch:()=>renderWeeklyUnitChecklist(),weeklySupplierSearch:()=>renderWeeklySupplierChecklist(),weeklyPurchaseSearch:()=>renderWeeklyPurchasePlan(),
    purchaseFlowSearch:()=>renderPurchaseFlow(),cdMoveSearch:()=>renderCdMoveProductList(),cdInboundSearch:()=>renderCdInboundPurchases(),expiryPlanogramSearch:()=>renderExpiryPlanogramProducts(),
    gsearch:()=>renderGroups(),stockDiffSearch:()=>renderStockDifferenceAnalysis(),stockMgmtUnitSearch:()=>renderStockManagementByUnit(),productUnitSearch:()=>renderProductUnitChecklist()
  };
  const changeMap={productsBySupplierFilter:()=>renderProductsBySupplier(),supplierRegistryFilter:()=>loadSupplierProductSelection(),stockDiffUnit:()=>renderStockDifferenceAnalysis(),stockDiffOnly:()=>renderStockDifferenceAnalysis(),stockMgmtUnitSelect:()=>renderStockManagementByUnit(),stockMgmtUnitStockFilter:()=>renderStockManagementByUnit(),stockMgmtUnitExpiryFilter:()=>renderStockManagementByUnit(),stockMgmtUnitStatusFilter:()=>renderStockManagementByUnit(),expiryFilter:()=>renderExpiry(),expiryRange:()=>renderExpiry(),cdInboundSupplier:()=>renderCdInboundPurchases(),cdInboundUnit:()=>renderCdInboundPurchases(),cdInboundStatus:()=>renderCdInboundPurchases()};
  for(const [id,fn] of Object.entries(inputMap)){const el=$("#"+id);if(el&&!el.dataset.robustFilter){el.dataset.robustFilter="1";el.addEventListener("input",()=>Promise.resolve(fn()).catch(e=>console.error("FILTRO "+id,e)))}}
  for(const [id,fn] of Object.entries(changeMap)){const el=$("#"+id);if(el&&!el.dataset.robustChange){el.dataset.robustChange="1";el.addEventListener("change",()=>Promise.resolve(fn()).catch(e=>console.error("FILTRO "+id,e)))}}
}
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",()=>setTimeout(installRobustFilterBindings,0));else setTimeout(installRobustFilterBindings,0);
...
async function cdInboundRows(){const purchases=await all("purchases"),rows=[];for(const p of purchases)for(const it of(p.items||[]))if(it.route==="CD"&&it.unitId&&it.status!=="SUPPLIED")rows.push({...it,purchaseId:p.id,purchaseAt:p.at||p.date||""});return rows.sort((a,b)=>String(b.purchaseAt).localeCompare(String(a.purchaseAt)))}

async function renderCdInboundPurchases(){
  const box=$("#cdInboundPurchases");if(!box)return;
  const purchases=await all("purchases"),rows=[];
  for(const p of purchases.filter(x=>x.source==="WEEKLY"&&x.status!=="COMPLETED"))for(const it of(p.items||[])){
    if(it.route!=="CD"||["SUPPLIED","CUT"].includes(it.status))continue;
    rows.push({...it,purchaseId:p.id,purchaseAt:p.at||""})
  }
  const q=normalizeText($("#cdInboundSearch")?.value||"");
  const filtered=rows.filter(x=>!q||normalizeText([x.product,x.ean,x.supplier,x.unit,x.purchaseId].join(" ")).includes(q));
  box.innerHTML=filtered.length?`<div class="dtable"><table><thead><tr><th>Compra</th><th>Fornecedor</th><th>Condomínio</th><th>Produto</th><th>Pedida</th><th>Recebida</th><th>Conferência</th><th>Validades</th><th>Ação</th></tr></thead><tbody>${filtered.map(x=>`<tr><td>${esc(x.purchaseId)}</td><td>${esc(x.supplier||"—")}</td><td>${esc(x.unit)}</td><td><b>${esc(x.product)}</b><br><small>${esc(x.ean)}</small></td><td>${qtyFmt(x.qty)}</td><td>${x.receivedQty===null||x.receivedQty===undefined?"—":qtyFmt(x.receivedQty)}</td><td>${esc(receivedStatus(x))}</td><td>${(x.expiryLots||[]).map(l=>`${qtyFmt(l.qty)} • ${esc(l.expiry)}${l.lot?` • ${esc(l.lot)}`:""}`).join("<br>")||"—"}</td><td><button onclick="openPurchaseFlowForItem('${x.unitId}','${x.supplier||""}','${x.ean}')">Conferir</button></td></tr>`).join("")}</tbody></table></div>`:'<p class="muted">Nenhuma mercadoria via CD aguardando conferência.</p>'
}
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.50.json",{cache:"no-store"});
...
async function saveExpiry(){
  const ean=$("#eean").value.replace(/\D/g,""),p=await prod(ean),u=$("#eunit").value,entries=[{date:$("#edate").value,qty:Math.max(0,Math.round(+$("#eqty").value||0)),lot:$("#elot")?.value.trim()||""},{date:$("#edate2")?.value||"",qty:Math.max(0,Math.round(+$("#eqty2")?.value||0)),lot:$("#elot2")?.value.trim()||""}].filter(x=>x.date&&x.qty>0);
  ...
  $("#edate").value="";$("#edate2").value="";$("#eqty2").value=0;$("#elot2").value="";await renderExpiry()
}
