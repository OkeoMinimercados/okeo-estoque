...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=3.58.0",{updateViaCache:"none"});
    await reg.update();
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.58.json",{cache:"no-store"});
...
async function renderSupplierProductList(){
  ...
  const rows=await supplierVisibleProducts(),allProducts=(await all("products")).filter(p=>p.active!==false);
  const offers=(await all("supplierOffers")).filter(x=>x.supplierId===sid&&x.active!==false),offerSet=new Set(offers.map(x=>x.ean));
  supplierProductSelection=new Set(allProducts.filter(p=>p.supplierId===sid||normalizeText(p.supplier)===normalizeText(supplier.name)||offerSet.has(p.ean)).map(p=>p.ean));
  const linkedVisible=rows.filter(p=>supplierProductSelection.has(p.ean)).length;
  if(summary)summary.innerHTML=`<b>${supplierProductSelection.size}</b> produto(s) vinculados a <b>${esc(supplier.name)}</b> • alterações do checklist são salvas imediatamente`;
  ...
  ${rows.length?`<table><thead><tr><th>✓</th><th>Produto</th><th>EAN</th><th>Fornecedor atual</th></tr></thead><tbody>${rows.map(p=>{
    const checked=supplierProductSelection.has(p.ean);
  ...
  }).join("")}</tbody></table>`:'<p class="muted">Nenhum produto encontrado.</p>'}`
}
...
async function toggleSupplierProduct(ean,on){
  const sid=$("#supplierRegistryFilter")?.value;if(!sid)return alert("Selecione o fornecedor.");
  ...
  if(on){
    ...
    const p=await prod(ean);
    if(p&&(p.supplierId===sid||normalizeText(p.supplier||"")===normalizeText(supplier.name))){
      await clearSupplierForProduct(ean);
      await audit("FORNECEDOR_PRODUTO_REMOVIDO",{supplierId:sid,supplier:supplier.name,ean})
    }else{
      const offers=(await all("supplierOffers")).filter(o=>o.supplierId===sid&&o.ean===ean&&o.active!==false);
      for(const o of offers)await localPut("supplierOffers",{...o,active:false,updatedAt:new Date().toISOString()})
    }
  }
  await renderSupplierProductList();
...
async function setVisibleSupplierProducts(on){
  ...
  if(!confirm(on?`Vincular ${rows.length} produto(s) visíveis a ${supplier.name}?`:`Remover de ${supplier.name} os produtos visíveis que estejam vinculados a ele?`))return;
  for(const p of rows){
    if(on)await setExclusiveSupplierForProduct(supplier,p.ean,+p.pc||0,"CHECKLIST_FORNECEDOR_MASSA");
    else if(p.supplierId===sid||normalizeText(p.supplier||"")===normalizeText(supplier.name))await clearSupplierForProduct(p.ean)
  }
  await audit(on?"FORNECEDOR_PRODUTOS_VINCULADOS_MASSA":"FORNECEDOR_PRODUTOS_REMOVIDOS_MASSA",{supplierId:sid,supplier:supplier.name,count:rows.length});
