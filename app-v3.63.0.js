...
const OKEO_FILTER_CHANGE_ROUTES={
  ...
  dunit:()=>calcDemand(),
  dweeks:()=>{if($("#demandCalcMsg"))$("#demandCalcMsg").textContent="Período alterado. Clique em Calcular / atualizar demanda."}
};
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=3.63.0",{updateViaCache:"none"});
    await reg.update();
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.63.json",{cache:"no-store"});
