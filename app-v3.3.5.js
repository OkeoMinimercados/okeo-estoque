...
async function backendRequest(action,params={},method="POST"){
  ...
  if(method==="GET"){
    ...
    response=await fetch(url,{method:"POST",body:fd,cache:"no-store",redirect:"follow"})
  }
  if(!response.ok)throw new Error("HTTP "+response.status);
  const text=await response.text();
  let data;
  try{data=JSON.parse(text)}
  catch(e){throw new Error("A Base Central respondeu em formato inválido. Confira se a URL termina em /exec.")}
  return data
}

async function authPost(action,params={}){
  const data=await backendRequest(action,params,"POST");
  if(data.ok===false)throw new Error(data.error||"Falha de autenticação");
  return data
}
...
async function validateSession(token){
  ...
  try{return await authPost("validate_session",{token})}
...
async function backup(){const o={version:"3.3.5",createdAt:new Date().toISOString(),stores:{}};for(const s of SYNC_STORES)o.stores[s]=await all(s);const b=new Blob([JSON.stringify(o,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="OKEO_CORE_Backup_V3_3.json";a.click()}