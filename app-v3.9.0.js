...
async function saveMove(){
  ...
  $("#mmsg").textContent=`Movimentação registrada: ${p.name} • ${n2(signedQty)}`;$("#mean").value="";$("#mqty").value=1;$("#mnote").value="";$("#mprod").textContent="Informe o EAN";renderMoves()
}
...
<td><input type="number" step=".01" value="${+x.cost||0}" onchange="nfParsedDraft[${i}].cost=+this.value||0"></td>
...
async function loadDemandSnapshot(){
  ...
  try{
    ...
    m.textContent=`${files.length} arquivo(s) importado(s) • ${sourceRecords} registros lidos • ${ok} produtos/unidades consolidados${unmapped?` • ${unmapped} não mapeados`:""}.`
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    const status=await apiGet("status");add("Backend",status.version==="3.9.0",`Versão ${status.version||"?"}`);add("Sessão",!!currentSession?.token,currentSession?.profileName||currentSession?.role||"sem perfil");
...
async function backup(){const o={version:"3.9.0",createdAt:new Date().toISOString(),stores:{}};for(const s of SYNC_STORES)o.stores[s]=await all(s);const b=new Blob([JSON.stringify(o,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="OKEO_CORE_Backup_V3_6.json";a.click()}