...
async function showApp(){
  ...
  Promise.resolve().then(async()=>{
    try{
      await ensureReferenceDataAvailable("LOGIN_V343");
...
async function forceReferenceReload(reason="MANUAL"){
  ...
  try{
    ...
    await sanitizeKnownSeedRelations();
    localStorage.setItem("okeo_reference_bootstrap_v343",new Date().toISOString());
...
async function ensureReferenceDataAvailable(reason="AUTO"){
  ...
  const boot=localStorage.getItem("okeo_reference_bootstrap_v343");
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.43.json",{cache:"no-store"});
