...
async function runSystemSelfTest(){
  ...
  try{
    ...
    const status=await apiGet("status");add("Backend",status.version==="3.27.0",`Versão ${status.version||"?"}`);add("Sessão",!!currentSession?.token,currentSession?.profileName||currentSession?.role||"sem perfil");
...
async function backup(){const o={version:"3.27.0",createdAt:new Date().toISOString(),stores:{}};for(const s of SYNC_STORES)o.stores[s]=await all(s);const b=new Blob([JSON.stringify(o,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="OKEO_CORE_Backup_V3_27.json";a.click()}