...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=4.0.2",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="4.0.2"){
        localStorage.setItem("okeo_frontend_cache_version","4.0.2")
      }
    })
...
async function verifyBackendVersion(expected="4.0.2"){
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("4.0.2");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 4.0.2. Publique primeiro o backend 4.0.2.`;
      alert(`Antes da correção, publique o backend 4.0.2. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    const diag=await diagnoseDeviceSync();
    if(!diag)throw new Error("Não foi possível comparar Local × Central.");
    const lp=+diag.localCounts.products||0,cp=+diag.centralCounts.products||0;
    const ls=+diag.localCounts.suppliers||0,lo=+diag.localCounts.supplierOffers||0;
    if(lp<100||ls<1||lo<1){
      throw new Error(`Este dispositivo não parece ser a base de origem completa (produtos ${lp}, fornecedores ${ls}, vínculos ${lo}).`)
    }
    const mismatches=centralMismatchSummary(diag.localCounts,diag.centralCounts);
    if(!mismatches.length){
      if(msg)msg.textContent="Local e Central já estão alinhados. Nenhuma correção necessária.";
      return
    }
    const preview=mismatches.slice(0,8).map(x=>`${x.label}: ${x.local} local × ${x.central} central`).join("\n");
    const typed=prompt(
      `CORREÇÃO CONTROLADA DA BASE CENTRAL\n\n`+
      `${preview}${mismatches.length>8?`\n+ ${mismatches.length-8} bases divergentes`:""}\n\n`+
      `Este computador será considerado a fonte oficial. Será criado um backup antes da operação.\n`+
      `Registros centrais extras nos stores operacionais serão removidos para espelhar a base local.\n\n`+
    ...
    await backup();
    const allRows={};let total=0,sent=0,removed=0;
    for(const store of SYNC_STORES){
      allRows[store]=await all(store);
      total+=allRows[store].length
    }
    for(const store of SYNC_STORES){
      const rows=allRows[store];
      if(msg)msg.textContent=`Publicando ${DEVICE_SYNC_LABELS[store]||store} • ${sent}/${total}`;
      for(let i=0;i<rows.length;i+=80){
        const chunk=rows.slice(i,i+80).map(r=>sanitize(store,r));
        if(chunk.length)await apiPost("bulk_upsert",{store,payload:JSON.stringify(chunk)},{timeoutMs:20000});
        sent+=chunk.length
      }
    }
    if(msg)msg.textContent="Removendo registros centrais extras...";
    for(const store of SYNC_STORES){
    ...
    if(remaining.length){
      ...
      if(msg)msg.textContent=`Correção concluída com pendências: ${txt}`;
      throw new Error("Ainda existem divergências após o espelhamento.")
    }
    if(msg)msg.textContent=`Correção concluída: ${sent} registros publicados, ${removed} extras centrais removidos. Local = Central.`;
    await audit("REPARO_BASE_CENTRAL",{sent,removed,at:new Date().toISOString()});
    ...
    if(msg)msg.textContent="Falha na correção: "+e.message;
...
async function runDeepHealthCheck(){
  ...
  try{
    ...
    const status=await apiGet("status",{}, {timeoutMs:7000});
    add("Backend",status.version==="4.0.2",`publicado v${status.version||"?"} • esperado v4.0.2`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="4.0.2",`Versão ${status.version||"?"}`);
