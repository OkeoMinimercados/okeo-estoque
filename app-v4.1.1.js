...
const EXPECTED_BACKEND_VERSION="4.1.1";
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=4.1.1",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="4.1.1"){
        localStorage.setItem("okeo_frontend_cache_version","4.1.1")
      }
    })
...
function getCurrentRuntimeErrors(){
  ...
  return cutoff?errors.filter(e=>(Date.parse(e.at)||0)>cutoff):errors
}
...
async function verifyBackendVersion(expected="4.1.1"){
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("4.1.1");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 4.1.1.`;
      alert(`Publique primeiro o backend 4.1.1. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    ...
    const resumable=progress&&progress.version==="4.1.1";
    if(!resumable){
      ...
      await backup();
      progress={version:"4.1.1",storeIndex:0,rowIndex:0,sent:0,startedAt:new Date().toISOString()};
...
async function runDeepHealthCheck(){
  ...
  const tests=[],add=(name,ok,detail)=>tests.push({name,ok:!!ok,detail:String(detail??"")});
  try{
    ...
    const ui=auditUiBindings();
    add("Botões",ui.ok,ui.ok?`${ui.buttons} botões com ação`:`${ui.issues.length} problema(s)`);
    const invariants=await checkCriticalInvariants();
    add("Invariantes operacionais",invariants.ok,invariants.ok?"Todas preservadas":`${invariants.issues.length} problema(s): ${invariants.issues.slice(0,3).join(" | ")}`);

    const requiredViews=ADMIN_VIEWS;
    for(const v of requiredViews)add("Tela "+v,!!$("#"+v),$("#"+v)?"OK":"Ausente");
    ...
    const status=await apiGet("status",{}, {timeoutMs:7000});
    add("Backend",status.version==="4.1.1",`publicado v${status.version||"?"} • esperado v4.1.1`);
    ...
    add("Snapshot deste dispositivo",!status.productionMode||!status.snapshotEpoch||deviceEpoch===status.snapshotEpoch,status.productionMode?`dispositivo ${deviceEpoch.slice(0,8)||"nenhum"} • central ${String(status.snapshotEpoch||"").slice(0,8)}`:"pré-produção");

    const local=await getLocalStoreCounts(),
          epochAligned=!!status.productionMode&&!!status.snapshotEpoch&&deviceEpoch===status.snapshotEpoch;
    if(epochAligned){
      ...
      for(const s of important)add("Base local "+s,true,`${+local[s]||0} registro(s) • dispositivo alinhado ao snapshot central`);
    }else{
      add("Sincronização de produção",!status.productionMode,status.productionMode?`Snapshot divergente: dispositivo ${deviceEpoch.slice(0,8)||"nenhum"} × central ${String(status.snapshotEpoch||"").slice(0,8)}`:"Pré-produção");
    }

    const recent=getCurrentRuntimeErrors();
    add("Erros atuais do navegador",recent.length===0,recent.length?`${recent.length} erro(s) após a preparação/atualização atual`:"Nenhum erro após a preparação atual");

    const failed=tests.filter(x=>!x.ok);
    out.innerHTML=`<div class="${failed.length?"integrity-bad":"integrity-ok"}"><b>Saúde ${failed.length?"COM PENDÊNCIAS":"APROVADA"}</b> • ${tests.length-failed.length}/${tests.length}</div>
      <div class="selftest-grid">${tests.map(t=>`<div class="selftest-item ${t.ok?"ok":"bad"}"><b>${t.ok?"✓":"✕"} ${esc(t.name)}</b><small>${esc(t.detail)}</small></div>`).join("")}</div>
      ${recent.length?`<details><summary>Erros recentes</summary>${recent.slice(0,10).map(e=>`<div class="runtime-error-row"><b>${esc(e.scope)}</b><span>${esc(e.message)}</span><small>${esc(e.at)}</small></div>`).join("")}</details>`:""}`
  }catch(e){
    rememberRuntimeError("DEEP_HEALTH_CHECK",e);
    out.innerHTML=`<div class="integrity-bad"><b>Revisão interrompida:</b> ${esc(e.message)}</div>`
  }
}
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="4.1.1",`Versão ${status.version||"?"}`);
