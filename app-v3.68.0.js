...
function sanitizeLoginPassword(v){
  return String(v??"").normalize("NFC").replace(/[\u200B-\u200D\u2060\uFEFF]/g,"").replace(/\u00A0/g," ")
}
async function authenticateLoginRobust(username,password){
  ...
  throw lastError||new Error("AUTH_INVALID")
}
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=3.68.0",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="3.68.0"){
        localStorage.setItem("okeo_frontend_cache_version","3.68.0")
      }
    })
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.68.json",{cache:"no-store"});
