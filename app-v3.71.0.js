...
async function authenticateLoginRobust(username,password){
  ...
  for(const pwd of [...new Set(candidates)]){
    try{return await authPost("login",{username:u,password:pwd})}
    catch(e){
      lastError=e;
...
async function login(){
  ...
  try{
    ...
    localStorage.removeItem("okeo_session");
    sessionStorage.removeItem("okeo_session");
    m.textContent="Falha no login: "+friendlyAuthError(e)+(sessionStorage.getItem("okeo_last_auth_reason")?` [${sessionStorage.getItem("okeo_last_auth_reason")}]`:"")+(sessionStorage.getItem("okeo_last_auth_backend")?` [BASE ${sessionStorage.getItem("okeo_last_auth_backend")}]`:"")
  }finally{
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=3.71.0",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="3.71.0"){
        localStorage.setItem("okeo_frontend_cache_version","3.71.0")
      }
    })
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v3.71.json",{cache:"no-store"});
