...
function bind(){
  ...
  if($("#invSelectorBtn"))$("#invSelectorBtn").onclick=()=>toggleSelectorPanel("invProductSelector","invSel",{unitId:$("#iunit").value,onChange:async s=>{if(s.selected.size===1){$("#iean").value=[...s.selected][0];await invLookup()}}});
...
const ADMIN_VIEWS=["home","stockmgmt","sales","inventory","control","entries","moves","cdmove","replenishment","expiry","groups","products","units","demand","settings"];
...
async function openStockMetricSelector(filter){
  const panel=$("#stockMgmtProductSelector");panel.classList.remove("hidden");await mountProductSelector("stockMgmtSel","stockMgmtProductSelector",{});const s=selectorState("stockMgmtSel");s.filter=filter;$("#stockMgmtSel_filter").value=filter;renderProductSelector("stockMgmtSel",{})
}
...
async function renderStockMovementAnalysis(){
  ...
  for(const x of rows){const q=Math.round(+x.qty||0);byType.set(x.type,(byType.get(x.type)||0)+q);const k=x.ean+"|"+(x.product||"");byProduct.set(k,(byProduct.get(k)||0)+Math.abs(q))}
...
async function exportStockMovementReport(){
  ...
  for(const x of rows)lines.push([x.at,x.type,x.ean,x.product,Math.round(+x.qty||0),x.from||"",x.to||"",String(x.note||"").replace(/;/g,",")].join(";"));
...
function renderCdMoveProductList(){
  const q=normalizeText($("#cdMoveSearch")?.value||""),rows=cdMoveRows.filter(x=>!q||normalizeText(x.product+" "+x.ean).includes(q));
  $("#cdMoveProductList").innerHTML=rows.map(x=>`<label class="check-row"><input type="checkbox" value="${x.ean}" ${cdMoveSelected.has(x.ean)?"checked":""} onchange="toggleCdMoveProduct('${x.ean}',this.checked)"><span><b>${esc(x.product)}</b><small>EAN ${x.ean} • Estoque ${qtyFmt(x.qty)} • ${money((+x.qty||0)*(+x.avgCost||+x.lastCost||0))}</small></span></label>`).join("")||'<p class="muted">Nenhum produto encontrado.</p>'
}
...
async function productSearchRows(context={}){
  const [products,stock,units,lots,demand]=await Promise.all([all("products"),all("stock"),all("units"),all("lots"),all("demandCurrent")]);
  const pm=new Map(products.map(x=>[x.ean,x])),um=new Map(units.map(x=>[x.id,x])),dm=new Map(demand.map(x=>[x.unitId+"|"+x.ean,x]));
  return products.filter(x=>x.active!==false).map(p=>{
    const st=context.unitId?stock.find(s=>s.unitId===context.unitId&&s.ean===p.ean):null,dr=context.unitId?dm.get(context.unitId+"|"+p.ean):null;
    const expiry7=context.unitId?lots.some(l=>l.unitId===context.unitId&&l.ean===p.ean&&+l.qty>0&&l.expiry&&((new Date(l.expiry+"T12:00:00")-new Date(new Date().toDateString()))/86400000)<=7&&((new Date(l.expiry+"T12:00:00")-new Date(new Date().toDateString()))/86400000)>=0):false;
    const hay=normalizeText([p.name,p.ean,p.vmPayName,...(p.aliases||[]),...(p.allNames||[]),p.supplier,p.segment,p.groupName,p.groupId].filter(Boolean).join(" "));
    return {...p,searchText:hay,stockQty:+st?.qty||0,status:dr?.status||"",need:+dr?.need||0,expiry7,unitName:context.unitId?(um.get(context.unitId)?.name||""):""}
  })
}
...
async function saveInventory(){
  ...
  $("#invmsg").textContent=`Contagem salva: ${p.name} • ${Math.round(prev)} → ${Math.round(q)} • Demanda/reposição da unidade atualizadas.`;$("#iean").value="";$("#iqty").value=0;$("#iprod").textContent="Digite ou leia o próximo EAN";$("#iean").focus();renderStock()
}
...
async function suggestPurchaseDistribution(){if($("#purchaseAllToCD")?.checked)return allocatePurchaseAllToCD();
  const e=$("#purchaseEAN").value.replace(/\D/g,""),qty=Math.max(0,+$("#purchaseQty").value||0),p=await prod(e);if(!p){await ensureProductOrOfferRegistration(e,{ean:e});return}if(qty<=0)return alert("Informe a quantidade.");
...
async function renderCdAllocationSuggestions(){
  const e=$("#cdSuggestEAN").value.replace(/\D/g,""),p=await prod(e),box=$("#cdAllocationSuggestions");if(!p){box.innerHTML=`<div class="integrity-warn">EAN ${esc(e)} não cadastrado. <button type="button" onclick="startAssistedProductRegistration('${e}')">Cadastrar produto</button></div>`;return}
  const ctx=await buildOperationalContext(),cd=ctx.units.find(x=>x.active!==false&&x.type==="CD"&&x.primaryCD!==false)||ctx.units.find(x=>x.active!==false&&x.type==="CD");
  if(!cd){box.innerHTML='<div class="integrity-bad">Nenhum CD ativo cadastrado.</div>';return}
  const cdStock=+(ctx.stockByKey.get(cd.id+"|"+e)?.qty||0),markets=ctx.units.filter(x=>x.active!==false&&x.type!=="CD");
  const rank={RUPTURA:0,"REPOSIÇÃO":1,OK:2,"SEM DEMANDA":3},rows=markets.map(u=>({...needFromContext(ctx,u.id,e),unitId:u.id,unit:u.name})).filter(x=>x.need>0).sort((a,b)=>rank[a.status]-rank[b.status]||b.need-a.need);
  let remaining=cdStock;const alloc=rows.map(x=>{const suggested=Math.min(remaining,x.need);remaining-=suggested;return {...x,suggested}});
  box.innerHTML=`<div class="metriccards"><div class="metric"><span>Produto</span><b>${esc(p.name)}</b></div><div class="metric"><span>Estoque CD</span><b>${n2(cdStock)}</b></div><div class="metric"><span>Saldo após sugestões</span><b>${n2(remaining)}</b></div></div>
  <div class="dtable"><table><thead><tr><th>Destino</th><th>Status</th><th>Estoque</th><th>Nível de Alerta</th><th>Estoque Ideal</th><th>Necessidade</th><th>Sugestão CD</th></tr></thead><tbody>${alloc.map(x=>`<tr><td>${esc(x.unit)}</td><td>${esc(x.status)}</td><td>${n2(x.stock)}</td><td>${n2(x.alert)}</td><td>${n2(x.ideal)}</td><td>${n2(x.need)}</td><td><b>${n2(x.suggested)}</b></td></tr>`).join("")}</tbody></table></div>`
}
...
async function xlsxDemandRows(file){
  const entries=await unzipEntries(await file.arrayBuffer()),dec=new TextDecoder();
  const ssxml=entries.get("xl/sharedStrings.xml"),shared=[];
  ...
  for(const rm of sx.matchAll(/<row\b[^>]*>([\s\S]*?)<\/row>/g)){
    const arr=[];for(const cm of rm[1].matchAll(/<c\b([^>]*)>([\s\S]*?)<\/c>/g)){const attrs=cm[1],body=cm[2],ref=/\br="([^"]+)"/.exec(attrs)?.[1]||"",type=/\bt="([^"]+)"/.exec(attrs)?.[1]||"",v=/<v>([\s\S]*?)<\/v>/.exec(body)?.[1]??"",inline=/<t\b[^>]*>([\s\S]*?)<\/t>/.exec(body)?.[1],col=xlsxCellCol(ref);let val=inline!=null?xmlDecode(inline):xmlDecode(v);if(type==="s")val=shared[+v]??"";arr[col]=val}rows.push(arr.map(x=>x??""))
  }
  if(rows.length<2)throw new Error(`${file.name}: planilha sem linhas de dados.`);
  const norm=s=>String(s||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]/g,""),heads=rows[0].map(norm),idx=(...names)=>{for(const n of names){const i=heads.indexOf(norm(n));if(i>=0)return i}return -1};
  const ix={unit:idx("unit","unidade","condominio","cnpj","lojacnpj","cnpjloja"),unitId:idx("unitId","idunidade"),ean:idx("ean","codigo","codigodebarras","gtin","produtoean"),product:idx("product","produto","descricao","nomeproduto"),historicalQty:idx("historicalQty","quantidade","qtd","vendas","quantidadevendida"),historicalWeeks:idx("historicalWeeks","semanas"),averageWeekly:idx("averageWeekly","mediasemanal","demandasemanal"),peakWeekly:idx("peakWeekly","picosemanal"),periodStart:idx("periodStart","inicio","datainicial"),periodEnd:idx("periodEnd","fim","datafinal")};
  if(ix.ean<0)throw new Error(`${file.name}: não encontrei coluna de EAN/código de barras.`);
  return rows.slice(1).map(c=>{const num=i=>i<0?0:+String(c[i]||"0").replace(/\./g,"").replace(",",".")||0,val=i=>i<0?"":c[i]||"";return {unit:val(ix.unit),unitId:val(ix.unitId),ean:val(ix.ean),product:val(ix.product),historicalQty:num(ix.historicalQty),historicalWeeks:num(ix.historicalWeeks),averageWeekly:num(ix.averageWeekly),peakWeekly:num(ix.peakWeekly),periodStart:val(ix.periodStart),periodEnd:val(ix.periodEnd)}})
}
async function readDemandHistoryFile(f){
  const name=String(f.name||"").toLowerCase(),bytes=new Uint8Array(await f.arrayBuffer());
  if(name.endsWith(".xlsx")||(bytes.length>=2&&bytes[0]===0x50&&bytes[1]===0x4b))return {records:await xlsxDemandRows(f),source:f.name};
  const text=new TextDecoder("utf-8").decode(bytes).replace(/^\uFEFF/,"").trim();
  if(name.endsWith(".csv"))return {records:csvDemandRows(text),source:f.name};
  if(!name.endsWith(".json"))throw new Error(`${f.name}: formato não suportado. Use XLSX, JSON ou CSV.`);
  let j;try{j=JSON.parse(text)}catch(e){throw new Error(`${f.name}: não é um JSON válido. Se for planilha Excel, salve como CSV.`)}
  const records=Array.isArray(j)?j:(Array.isArray(j.records)?j.records:null);
  if(!records)throw new Error(`${f.name}: JSON sem lista de registros.`);
  return {records,periodStart:j.periodStart||"",periodEnd:j.periodEnd||"",source:f.name}
}
async function loadDemandSnapshot(){
  const files=[...($("#demandSnapshotFile").files||[])],m=$("#demandSnapshotMsg");if(!files.length)return alert("Selecione um ou mais arquivos JSON ou CSV.");
  const units=await all("units"),agg=new Map();let sourceRecords=0,unmapped=0;const errors=[],startedAt=new Date().toISOString();
  m.textContent=`Validando ${files.length} arquivo(s) em memória temporária...`;
  for(const f of files){
    try{
      const j=await readDemandHistoryFile(f);
      for(const r of j.records){
        const ut=String(r.unit||"").trim(),ui=String(r.unitId||"").trim(),u=units.find(x=>String(x.name||"").trim()===ut||String(x.id||"")===ui||String(x.cnpj||"").replace(/\D/g,"")===ut.replace(/\D/g,""));
        if(!u){unmapped++;continue}const ean=String(r.ean||"").replace(/\D/g,"");if(!ean)continue;
        const key=u.id+"|"+ean,o=agg.get(key)||{unitId:u.id,ean,product:r.product||"",historicalQty:0,historicalWeeks:0,peakWeekly:0,periodStart:"",periodEnd:""};
        const ps=r.periodStart||j.periodStart||"",pe=r.periodEnd||j.periodEnd||"",weeks=+r.historicalWeeks||((ps&&pe)?Math.max(1,Math.round((new Date(pe)-new Date(ps))/604800000)+1):0),qty=+r.historicalQty||((+r.averageWeekly||0)*weeks);
        o.historicalQty+=qty;o.historicalWeeks+=weeks;o.peakWeekly=Math.max(o.peakWeekly,+r.peakWeekly||0);o.product=o.product||r.product||"";
        if(ps&&(!o.periodStart||ps<o.periodStart))o.periodStart=ps;if(pe&&(!o.periodEnd||pe>o.periodEnd))o.periodEnd=pe;agg.set(key,o);sourceRecords++
      }
    }catch(e){errors.push(e.message)}
  }
  if(!agg.size){m.textContent=`Nenhum arquivo válido foi processado. O snapshot anterior foi preservado. ${errors.join(" | ")}`;return}
  try{
    const staged=[...agg].map(([id,r])=>{const avg=r.historicalWeeks>0?r.historicalQty/r.historicalWeeks:0;return{id,unitId:r.unitId,ean:r.ean,product:r.product,historicalQty:Math.round(r.historicalQty),historicalWeeks:r.historicalWeeks,averageWeekly:avg,peakWeekly:Math.round(r.peakWeekly),alertLevel:Math.ceil(avg*.5),idealStock:Math.ceil(avg),periodStart:r.periodStart,periodEnd:r.periodEnd,calculationVersion:"3.12.0",processedAt:new Date().toISOString(),updatedAt:new Date().toISOString()}});
    if(!staged.length)throw new Error("Nenhum resultado consolidado.");
    m.textContent=`${staged.length} resultados calculados. Gravando snapshot compacto...`;
    await clearStore("demandBase");await clearStore("demandCurrent");for(const row of staged)await localPut("demandBase",row);
    const persisted=await all("demandBase");if(persisted.length!==staged.length)throw new Error("Falha na verificação do snapshot.");
    await recalculateDemandCurrent(units.filter(x=>x.active!==false&&x.type!=="CD").map(x=>x.id));const current=await all("demandCurrent");
    if(!current.length&&staged.length)throw new Error("Demanda operacional não foi gerada.");
    const starts=staged.map(x=>x.periodStart).filter(Boolean).sort(),ends=staged.map(x=>x.periodEnd).filter(Boolean).sort(),meta={processedAt:new Date().toISOString(),startedAt,filesReceived:files.length,filesValid:files.length-errors.length,sourceRecords,consolidatedRecords:staged.length,operationalRecords:current.length,unmapped,periodStart:starts[0]||"",periodEnd:ends.length?ends[ends.length-1]:"",calculationVersion:"3.12.0",rawBasePurged:true,errors};
    await localPut("settings",{id:"DEMAND_PROCESSING_META",value:meta,updatedAt:meta.processedAt});await audit("DEMANDA_SNAPSHOT_PROCESSADO",meta);
    agg.clear();await calcDemand();await renderDemandStorageStatus();
    m.textContent=`Demanda processada: ${staged.length} resultados consolidados • base bruta descartada após validação${errors.length?` • ${errors.length} arquivo(s) ignorado(s)`:""}.`
  }catch(e){console.error(e);m.textContent="Falha ao finalizar: "+e.message+" O sistema não mantém a base bruta importada."}
}
...
async function renderReplenishmentEditor(){const selectedFilter=selectorState("repSel").selected;const repRows=selectedFilter.size?repDraftRows.filter(x=>selectedFilter.has(x.ean)):repDraftRows;
...
async function startControlPoint(){
  ...
  for(const s of stocks)await localPut("controlPointItems",{id:cid+"|"+s.ean,controlId:cid,unitId,ean:s.ean,product:s.product,predicted:+s.qty||0,observed:+s.qty||0,avgCost:+s.avgCost||0,difference:0,valueDifference:0,updatedAt:createdAt});
  activeControlId=cid;await audit("PONTO_CONTROLE_INICIADO",{controlId:cid,unitId,countedAt});renderControlPoint()
}

async function addSelectedControlProducts(){
  if(!activeControlId)return alert("Inicie um Ponto de Controle primeiro.");
...
async function approveControlPoint(){
  if(!activeControlId)return alert("Nenhum ponto em andamento.");if(!confirm("Aprovar o ponto de controle? O observado vira a nova referência a partir da data/hora da contagem."))return;
  const cp=await get("controlPoints",activeControlId),items=(await all("controlPointItems")).filter(x=>x.controlId===activeControlId),now=new Date().toISOString(),baselineAt=cp.countedAt||now;
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    const status=await apiGet("status");add("Backend",status.version==="3.15.0",`Versão ${status.version||"?"}`);add("Sessão",!!currentSession?.token,currentSession?.profileName||currentSession?.role||"sem perfil");
...
async function backup(){const o={version:"3.15.0",createdAt:new Date().toISOString(),stores:{}};for(const s of SYNC_STORES)o.stores[s]=await all(s);const b=new Blob([JSON.stringify(o,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="OKEO_CORE_Backup_V3_6.json";a.click()}