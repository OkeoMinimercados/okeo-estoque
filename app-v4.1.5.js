...
const EXPECTED_BACKEND_VERSION="4.1.5";
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=4.1.5",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="4.1.5"){
        localStorage.setItem("okeo_frontend_cache_version","4.1.5")
      }
    })
...
async function verifyBackendVersion(expected="4.1.5"){
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("4.1.5");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 4.1.5.`;
      alert(`Publique primeiro o backend 4.1.5. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    ...
    const resumable=progress&&progress.version==="4.1.5";
    if(!resumable){
      ...
      await backup();
      progress={version:"4.1.5",storeIndex:0,rowIndex:0,sent:0,startedAt:new Date().toISOString()};
...
async function healthBackendProbe(timeoutMs=4500){
  ...
  try{
    const status=await backendRequest("status",{},"GET",{timeoutMs});
    ...
    return {ok:status?.ok===true,status,ms:Math.round(performance.now()-started),timeout:false}
  }catch(e){
    const timeout=isTimeoutError(e);
    console.warn("HEALTH_REMOTE_PROBE",e);
    return {ok:false,status:null,ms:Math.round(performance.now()-started),timeout,error:String(e?.message||e)}
  }
}
...
async function runDeepHealthCheck(){
  ...
  if(remote.ok){
    ...
    add("Base Central online",true,`resposta ${remote.ms} ms`);
    add("Backend",status.version==="4.1.5",`publicado v${status.version||"?"} • esperado v4.1.5`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="4.1.5",`Versão ${status.version||"?"}`);
