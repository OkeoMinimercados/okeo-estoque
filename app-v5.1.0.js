...
const FRONTEND_VERSION="5.1.0";
const EXPECTED_BACKEND_VERSION="5.1.0";
...
async function bridgeRequest(url,action,params={},timeoutMs=15000){
  const clean=String(url||"").trim();
  if(!clean)throw new Error("Base Central não configurada.");
  const id=(globalThis.crypto?.randomUUID?.()||("br_"+Date.now()+"_"+Math.random().toString(36).slice(2))),
        iframe=document.createElement("iframe"),form=document.createElement("form"),
        target="okeo_bridge_"+id.replace(/[^a-z0-9]/gi,"");
  iframe.name=target;iframe.style.display="none";iframe.setAttribute("aria-hidden","true");
  document.body.appendChild(iframe);
  form.method="POST";form.action=clean;form.target=target;form.style.display="none";
  const fields={action,bridge:"1",bridgeId:id};
  Object.entries(params||{}).forEach(([k,v])=>fields[k]=typeof v==="string"?v:JSON.stringify(v));
  ...
  return await new Promise((resolve,reject)=>{
    ...
    const onMessage=e=>{
      const d=e.data||{};
      if(e.source!==iframe.contentWindow||d.channel!=="OKEO_GAS_BRIDGE_V1"||d.id!==id)return;
      try{finish(resolve,decodeBridgePayload(d.payload))}
      catch(err){finish(reject,new Error("Resposta inválida da Base Central"))}
    };
    window.addEventListener("message",onMessage);
    const timer=setTimeout(()=>finish(reject,new Error("TIMEOUT_BASE_CENTRAL")),Math.max(3000,+timeoutMs||15000));
    try{form.submit()}catch(e){finish(reject,e)}
  })
}
async function probeBackendUrlDirect(url,timeoutMs=10000){
  return await bridgeRequest(url,"status",{},timeoutMs)
}

async function setBackendUrl(url){
  const requested=String(url||"").trim();
  if(requested&&requested!==DEFAULT_BACKEND_URL){
    throw new Error("A Base Central está bloqueada na implantação oficial desta versão.")
  }
  const probe=await probeBackendUrlDirect(DEFAULT_BACKEND_URL,12000),
        version=String(probe.version||"");
  if(version!==EXPECTED_BACKEND_VERSION){
    throw new Error(`Backend oficial incompatível: ${version||"indisponível"}; esperado ${EXPECTED_BACKEND_VERSION}.`)
  }
  await resolveBackendUrl();
  invalidateBackendConnectionCache();
  backendCompatibility={checkedAt:Date.now(),version,ok:true,productionMode:!!probe.productionMode,snapshotEpoch:String(probe.snapshotEpoch||""),deploymentTail:backendDeploymentTail(DEFAULT_BACKEND_URL)};
...
async function backendRequest(action,params={},method="POST",options={}){
  const url=options.url||await resolveBackendUrl();
  return await bridgeRequest(url,action,params,options.timeoutMs||12000)
}
...
async function createFirstAdmin(){
  ...
  try{
    const r=await authPost("bootstrap_admin",{username,password:p1},{url:DEFAULT_BACKEND_URL,timeoutMs:15000});
...
async function authenticateLoginRobust(username,password){
  ...
  for(const pwd of [...new Set(candidates)]){
    try{return await authPost("login",{username:u,password:pwd},{url:DEFAULT_BACKEND_URL,timeoutMs:12000})}
    catch(e){
      lastError=e;
...
async function login(){
  ...
  try{
    ...
    if(e?.message==="TIMEOUT_BASE_CENTRAL"){
      m.textContent="Falha no login: a Base Central demorou mais de 12 segundos para responder. Tente novamente.";
    }else{
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=5.1.0",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="5.1.0"){
        localStorage.setItem("okeo_frontend_cache_version","5.1.0")
      }
    })
...
window.addEventListener("online",()=>runControlledSyncCycle("ONLINE",{pullCore:true}));
...
async function loadSeedUnitRelations(){
  ...
  try{
    const r=await fetch("./seed-planograms-v4.0.json",{cache:"no-store"});
...
async function verifyBackendVersion(expected="5.1.0"){
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("5.1.0");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 5.1.0.`;
      alert(`Publique primeiro o backend 5.1.0. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    ...
    const resumable=progress&&progress.version==="5.1.0";
    if(!resumable){
      ...
      await backup();
      progress={version:"5.1.0",storeIndex:0,rowIndex:0,sent:0,startedAt:new Date().toISOString()};
...
async function runDeepHealthCheck(){
  ...
  if(remote.ok){
    ...
    add("Backend",status.version==="5.1.0",`publicado v${status.version||"?"} • esperado v5.1.0`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="5.1.0",`Versão ${status.version||"?"}`);
...
async function downloadCentralBaseToDevice(){
  ...
  try{
    ...
    for(const store of SYNC_STORES){
      ...
      const r=await apiGet("list",{store},{timeoutMs:30000});
      if(!Array.isArray(r.rows))throw new Error(`Resposta inválida em ${store}`);
      const checksum=localSnapshotChecksum(r.rows);
      if(+expected.counts?.[store]!==r.rows.length||String(expected.checksums?.[store]||"")!==checksum){
        throw new Error(`SNAPSHOT_INVALIDO ${store}: esperado ${expected.counts?.[store]}/${expected.checksums?.[store]} × recebido ${r.rows.length}/${checksum}`)
      }
      snapshot[store]=r.rows;
      loaded+=r.rows.length
    }
