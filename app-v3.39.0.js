...
let supplierDraftTouched=false,referenceReloadBusy=false;
...
async function showApp(){
  ...
  applyAccessProfile();
  try{await ensureReferenceDataAvailable("LOGIN_V339");await reconcileAllProductUnitRelations();await selectors()}catch(e){console.warn("Carga inicial crítica",e)}
...
async function forceReferenceReload(reason="MANUAL"){
  ...
  try{
    await processQueue();
    const stores=["products","units","planograms","stock","demandBase","demandCurrent","suppliers","supplierOffers","purchases","invoices"];
    let loaded=0;
    for(const store of stores){
      try{const r=await apiGet("list",{store});if(Array.isArray(r.rows)){for(const row of r.rows)await put(store,row);loaded+=r.rows.length}}catch(e){console.warn("Recarga integral",store,e)}
    }
    await hydrateSupplierCatalog();
    localStorage.setItem("okeo_reference_bootstrap_v339",new Date().toISOString());
...
async function ensureReferenceDataAvailable(reason="AUTO"){
  ...
  const boot=localStorage.getItem("okeo_reference_bootstrap_v339");
...
async function hydrateSupplierCatalog(){
  ...
  const known=new Map(suppliers.map(x=>[normalizeText(x.name),x]));
  const names=[];
  for(const p of products)if(p.supplier)names.push(p.supplier);
  for(const o of offers)if(o.supplierName)names.push(o.supplierName);
  for(const x of purchases)if(x.supplier)names.push(x.supplier);
  for(const x of invoices)if(x.supplier)names.push(x.supplier);
  const now=new Date().toISOString();
  for(const raw of names){const name=String(raw||"").trim(),n=normalizeText(name);if(!name||known.has(n))continue;const row={id:id("sup"),name,normalizedName:n,active:true,source:"RECONCILIACAO_CATALOGO",createdAt:now,updatedAt:now};await localPut("suppliers",row);known.set(n,row)}
  return [...known.values()]
}
...
async function canonicalUnitProductEans(unitId,{repair=true}={}){
  ...
  for(const st of stock)if(pm.has(st.ean))eans.add(st.ean);
...
async function renderSupplierRegistryAdmin(){
  await hydrateSupplierCatalog();await refreshSupplierRegistryFilter(true);const q=normalizeText($("#supplierRegistrySearch")?.value||""),suppliers=(await all("suppliers")).filter(x=>normalizeText(x.name).includes(q)).sort((a,b)=>a.name.localeCompare(b.name)),offers=await all("supplierOffers"),box=$("#supplierRegistryAdminList");if(!box)return;
...
async function saveSupplierProductCoverage(){
  const sid=$("#supplierRegistryFilter")?.value;if(!sid)return alert("Selecione o fornecedor.");const supplier=await get("suppliers",sid);if(!supplier)return;await persistSupplierCoverageSelection(supplier,supplierProductSelection);
