...
function bind(){
  ...
  if($("#weeklyUnitFilter"))$("#weeklyUnitFilter").onchange=renderWeeklyPurchasePlan;
  if($("#weeklySupplierFilter"))$("#weeklySupplierFilter").onchange=renderWeeklyPurchasePlan;
  if($("#weeklyCoverageDays"))$("#weeklyCoverageDays").onchange=()=>{weeklyPurchaseRows=[];renderWeeklyPurchasePlan()};
  if($("#purchaseOpenReplenishment"))$("#purchaseOpenReplenishment").onclick=()=>view("replenishment");
  if($("#purchaseRefreshReplenishment"))$("#purchaseRefreshReplenishment").onclick=renderPurchaseReplenishmentSummary;
...
async function getPhysicalCountUnitProducts(unitId){
  const [products,stock,demand,planogram]=await Promise.all([all("products"),all("stock"),all("demandCurrent"),byIndex("planograms","unitId",unitId)]),
        ...
        pg=new Set(planogram.filter(x=>x.active!==false).map(x=>x.ean));
  return products.filter(p=>p.active!==false).map(p=>{
    const s=sm.get(p.ean),d=dm.get(p.ean),inPlanogram=pg.has(p.ean);
    return {ean:p.ean,product:p.name,p,predicted:Math.round(+s?.qty||0),observed:null,avgCost:+s?.avgCost||+s?.lastCost||+p.pc||0,status:d?.status||"",selected:inPlanogram||(+s?.qty||0)>0,inPlanogram}
...
function renderPhysicalCountProductList(){
  ...
  box.innerHTML=`<div class="physical-list-actions"><button type="button" class="secondary" onclick="setAllPhysicalSelection(true)">Selecionar visíveis</button><button type="button" class="secondary" onclick="setAllPhysicalSelection(false)">Desmarcar visíveis</button><span><b>${qtyFmt(rows.length)}</b> produtos visíveis • <b>${qtyFmt(physicalCountRows.length)}</b> na Base Mestre</span></div>
...
async function setStockMgmtMode(mode){stockMgmtViewMode=mode==="unit"?"unit":"general";$("#stockMgmtGeneralPanel")?.classList.toggle("hidden",stockMgmtViewMode==="unit");$("#stockMgmtUnitPanel")?.classList.toggle("hidden",stockMgmtViewMode!=="unit");$("#stockMgmtTabGeneral")?.classList.toggle("active",stockMgmtViewMode==="general");$("#stockMgmtTabUnit")?.classList.toggle("active",stockMgmtViewMode==="unit");if(stockMgmtViewMode==="unit")await renderStockManagementByUnit()}
async function renderStockManagementByUnit(){
  ...
  const relevantEans=new Set(plan.map(x=>x.ean));
  for(const s of stock.filter(x=>x.unitId===unitId))if((+s.qty||0)!==0)relevantEans.add(s.ean);
  for(const d of (ctx.demandByUnit.get(unitId)||[]))if(d.ean)relevantEans.add(d.ean);
  for(const d of (ctx.currentDemandByUnit.get(unitId)||[]))for(const e of(d.eans||[d.ean]))if(e)relevantEans.add(e);
...
async function renderEntries(){
  ...
  renderPurchaseItems();renderPurchaseDistribution();if(weeklyPurchaseRows.length)await populateWeeklyPurchaseFilters();renderWeeklyPurchasePlan();await renderPurchaseReplenishmentSummary();
...
function showPurchaseTab(t){
  ...
  if(t==="weekly")renderWeeklyPurchasePlan();if(t==="replenishment")renderPurchaseReplenishmentSummary()
}
...
let weeklyPurchaseRows=[],weeklyPurchaseView="UNIT",weeklyPurchaseGeneratedAt="";
function weeklyPurchaseSelectedRows(){
  const q=normalizeText($("#weeklyPurchaseSearch")?.value||""),unit=$("#weeklyUnitFilter")?.value||"ALL",supplier=$("#weeklySupplierFilter")?.value||"ALL";
  return weeklyPurchaseRows.filter(x=>(unit==="ALL"||x.unitId===unit)&&(supplier==="ALL"||x.supplier===supplier)&&(!q||normalizeText([x.product,x.ean,x.unit,x.supplier].join(" ")).includes(q)))
}
async function populateWeeklyPurchaseFilters(){
  const units=(await all("units")).filter(x=>x.active!==false&&x.type!=="CD").sort((a,b)=>a.name.localeCompare(b.name)),suppliers=[...new Set(weeklyPurchaseRows.map(x=>x.supplier).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  const us=$("#weeklyUnitFilter"),ss=$("#weeklySupplierFilter");if(us){const old=us.value||"ALL";us.innerHTML='<option value="ALL">Todos os condomínios</option>'+units.map(u=>`<option value="${u.id}">${esc(u.name)}</option>`).join("");us.value=[...us.options].some(o=>o.value===old)?old:"ALL"}if(ss){const old=ss.value||"ALL";ss.innerHTML='<option value="ALL">Todos os fornecedores</option>'+suppliers.map(s=>`<option value="${esc(s)}">${esc(s)}</option>`).join("");ss.value=[...ss.options].some(o=>o.value===old)?old:"ALL"}
}
