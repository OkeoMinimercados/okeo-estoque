...
function bind(){
  ...
  $("#mean").oninput=moveProd;$("#mtype").onchange=moveTypeUI;$("#msave").onclick=saveMove;if($("#cdSuggestBtn"))$("#cdSuggestBtn").onclick=renderCdAllocationSuggestions;
...
async function readDemandHistoryFile(f){
  ...
  if(bytes.length>=2&&bytes[0]===0x50&&bytes[1]===0x4b)throw new Error(`${f.name}: arquivo compactado/Excel detectado. Salve como CSV ou use o JSON exportado pelo OKEO.`);
  ...
  if(!name.endsWith(".json"))throw new Error(`${f.name}: formato não suportado. Use JSON ou CSV.`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    const status=await apiGet("status");add("Backend",status.version==="3.12.0",`Versão ${status.version||"?"}`);add("Sessão",!!currentSession?.token,currentSession?.profileName||currentSession?.role||"sem perfil");
...
async function backup(){const o={version:"3.12.0",createdAt:new Date().toISOString(),stores:{}};for(const s of SYNC_STORES)o.stores[s]=await all(s);const b=new Blob([JSON.stringify(o,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="OKEO_CORE_Backup_V3_6.json";a.click()}