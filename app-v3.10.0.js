...
async function loadDemandSnapshot(){
  const files=[...($("#demandSnapshotFile").files||[])],m=$("#demandSnapshotMsg");if(!files.length)return alert("Selecione um ou mais arquivos JSON.");
  try{
    const units=await all("units"),agg=new Map();let sourceRecords=0,unmapped=0;
    for(const f of files){
      const j=JSON.parse(await f.text());if(!Array.isArray(j.records))throw new Error(`${f.name}: snapshot inválido.`);
      for(const r of j.records){
        const u=units.find(x=>x.name===r.unit||x.id===r.unitId);if(!u){unmapped++;continue}
        const ean=String(r.ean||"").replace(/\D/g,"");if(!ean)continue;
        const key=u.id+"|"+ean,old=agg.get(key)||{unitId:u.id,ean,product:r.product||"",historicalQty:0,historicalWeeks:0,peakWeekly:0,periodStart:"",periodEnd:""};
        const start=r.periodStart||j.periodStart||"",end=r.periodEnd||j.periodEnd||"";
        const weeks=+r.historicalWeeks||((start&&end)?Math.max(1,Math.round((new Date(end)-new Date(start))/604800000)+1):0);
        const qty=+r.historicalQty||((+r.averageWeekly||0)*weeks);
        old.historicalQty+=qty;old.historicalWeeks+=weeks;old.peakWeekly=Math.max(old.peakWeekly,+r.peakWeekly||0);old.product=old.product||r.product||"";
        if(start&&(!old.periodStart||start<old.periodStart))old.periodStart=start;if(end&&(!old.periodEnd||end>old.periodEnd))old.periodEnd=end;
        agg.set(key,old);sourceRecords++
      }
    }
    await clearStore("demandBase");await clearStore("demandCurrent");
    let ok=0;for(const [id,r] of agg){
      const avg=r.historicalWeeks>0?r.historicalQty/r.historicalWeeks:0;
      await localPut("demandBase",{id,unitId:r.unitId,ean:r.ean,product:r.product,historicalQty:r.historicalQty,historicalWeeks:r.historicalWeeks,averageWeekly:avg,peakWeekly:r.peakWeekly,alertLevel:Math.ceil(avg*.5),idealStock:Math.ceil(avg),periodStart:r.periodStart,periodEnd:r.periodEnd,updatedAt:new Date().toISOString()});ok++
    }
    await audit("SNAPSHOT_DEMANDA_IMPORTADO",{files:files.length,sourceRecords,consolidatedRecords:ok,unmapped});
    m.textContent=`${files.length} arquivo(s) importado(s) • ${sourceRecords} registros lidos • ${ok} produtos/unidades consolidados${unmapped?` • ${unmapped} não mapeados`:""} • recalculando...`;
    await recalculateDemandCurrent(units.filter(x=>x.active!==false&&x.type!=="CD").map(x=>x.id));
    await calcDemand();
    m.textContent=`Importação concluída: ${files.length} arquivo(s) • ${ok} registros consolidados${unmapped?` • ${unmapped} não mapeados`:""} • Demanda Inteligente atualizada.`
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    const status=await apiGet("status");add("Backend",status.version==="3.10.0",`Versão ${status.version||"?"}`);add("Sessão",!!currentSession?.token,currentSession?.profileName||currentSession?.role||"sem perfil");
...
async function backup(){const o={version:"3.10.0",createdAt:new Date().toISOString(),stores:{}};for(const s of SYNC_STORES)o.stores[s]=await all(s);const b=new Blob([JSON.stringify(o,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="OKEO_CORE_Backup_V3_6.json";a.click()}