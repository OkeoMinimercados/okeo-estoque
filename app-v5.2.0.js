...
const FRONTEND_VERSION="5.2.0";
const EXPECTED_BACKEND_VERSION="5.2.0";
...
const GAS_BRIDGE_CHANNEL="OKEO_NATIVE_BRIDGE_V1";
let gasBridgeFrame=null,gasBridgeReadyPromise=null,gasBridgeReadyAbort=null,gasBridgeClientId="",gasBridgeSeq=0,gasBridgePending=new Map();
function resetGasBridge(reason="RESET"){
  if(gasBridgeReadyAbort){try{gasBridgeReadyAbort(reason)}catch(e){}gasBridgeReadyAbort=null}
  for(const [id,p] of gasBridgePending){clearTimeout(p.timer);p.reject(new Error("BRIDGE_RESET_"+reason))}
  gasBridgePending.clear();
  if(gasBridgeFrame){try{gasBridgeFrame.remove()}catch(e){}}
  gasBridgeFrame=null;gasBridgeReadyPromise=null;gasBridgeClientId=""
}
function ensureGasBridge(timeoutMs=15000){
  if(gasBridgeReadyPromise)return gasBridgeReadyPromise;
  gasBridgeReadyPromise=new Promise((resolve,reject)=>{
    const clientId="cli_"+Date.now()+"_"+Math.random().toString(36).slice(2),iframe=document.createElement("iframe");
    let settled=false,timer=null;
    const cleanup=()=>{window.removeEventListener("message",onReady);if(timer)clearTimeout(timer);gasBridgeReadyAbort=null};
    const fail=err=>{if(settled)return;settled=true;cleanup();if(gasBridgeFrame===iframe){try{iframe.remove()}catch(e){}gasBridgeFrame=null;gasBridgeReadyPromise=null;gasBridgeClientId=""}reject(err)};
    const succeed=()=>{if(settled)return;settled=true;cleanup();resolve(iframe)};
    const onReady=e=>{const d=e.data||{};if(e.source!==iframe.contentWindow||d.channel!==GAS_BRIDGE_CHANNEL||d.type!=="ready"||d.clientId!==clientId)return;succeed()};
    gasBridgeReadyAbort=reason=>fail(new Error("BRIDGE_RESET_"+reason));
    gasBridgeClientId=clientId;gasBridgeFrame=iframe;
    iframe.id="okeoGasBridge";iframe.style.display="none";iframe.setAttribute("aria-hidden","true");
    window.addEventListener("message",onReady);
    timer=setTimeout(()=>fail(new Error("TIMEOUT_BASE_CENTRAL")),Math.max(5000,+timeoutMs||15000));
    iframe.src=DEFAULT_BACKEND_URL+(DEFAULT_BACKEND_URL.includes("?")?"&":"?")+"uiBridge=1&clientId="+encodeURIComponent(clientId)+"&v="+encodeURIComponent(FRONTEND_VERSION)+"&_="+Date.now();
...
window.addEventListener("message",e=>{
  const d=e.data||{};
  if(!gasBridgeFrame||e.source!==gasBridgeFrame.contentWindow||d.channel!==GAS_BRIDGE_CHANNEL||d.clientId!==gasBridgeClientId||d.type!=="response")return;
  const p=gasBridgePending.get(d.id);if(!p)return;
  gasBridgePending.delete(d.id);clearTimeout(p.timer);
  if(d.ok)p.resolve(d.result);else p.reject(new Error(String(d.error||"ERRO_BASE_CENTRAL")))
});
async function gasBridgeRequest(action,params={},timeoutMs=15000){
  const frame=await ensureGasBridge(Math.max(timeoutMs,15000)),id="rpc_"+(++gasBridgeSeq)+"_"+Date.now();
  return await new Promise((resolve,reject)=>{
    const timer=setTimeout(()=>{gasBridgePending.delete(id);reject(new Error("TIMEOUT_BASE_CENTRAL"));resetGasBridge("REQUEST_TIMEOUT")},Math.max(3000,+timeoutMs||15000));
    gasBridgePending.set(id,{resolve,reject,timer});
    try{frame.contentWindow.postMessage({channel:GAS_BRIDGE_CHANNEL,type:"request",clientId:gasBridgeClientId,id,action:String(action||""),params:params||{}},"*")}
    catch(e){gasBridgePending.delete(id);clearTimeout(timer);reject(e);resetGasBridge("POSTMESSAGE_ERROR")}
  })
}
async function probeBackendUrlDirect(url,timeoutMs=10000){return await gasBridgeRequest("status",{},timeoutMs)}
async function setBackendUrl(url){
  ...
  localStorage.setItem("okeo_backend_last_ok_version",version);localStorage.setItem("okeo_backend_last_ok_at",new Date().toISOString());
  return {url:DEFAULT_BACKEND_URL,status:probe}
}
async function backendRequest(action,params={},method="POST",options={}){
  return await gasBridgeRequest(action,params,options.timeoutMs||15000)
}
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=5.2.0",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="5.2.0"){
        localStorage.setItem("okeo_frontend_cache_version","5.2.0")
      }
    })
...
async function verifyBackendVersion(expected="5.2.0"){
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("5.2.0");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 5.2.0.`;
      alert(`Publique primeiro o backend 5.2.0. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    ...
    const resumable=progress&&progress.version==="5.2.0";
    if(!resumable){
      ...
      await backup();
      progress={version:"5.2.0",storeIndex:0,rowIndex:0,sent:0,startedAt:new Date().toISOString()};
...
async function runDeepHealthCheck(){
  ...
  if(remote.ok){
    ...
    add("Backend",status.version==="5.2.0",`publicado v${status.version||"?"} • esperado v5.2.0`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="5.2.0",`Versão ${status.version||"?"}`);
