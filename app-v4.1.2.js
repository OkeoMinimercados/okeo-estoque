...
const EXPECTED_BACKEND_VERSION="4.1.2";
...
async function startupPreflight(){
  ...
  try{
    ...
    if(errors.length)issues.push(`${errors.length} erro(s) técnico(s) recente(s) registrado(s).`);
    ...
    if(c.productionMode&&c.snapshotEpoch&&localEpoch!==c.snapshotEpoch){
      issues.push(`Snapshot deste dispositivo está desatualizado (${localEpoch||"nenhum"} → ${c.snapshotEpoch.slice(0,8)}). Atualize pela Base Central antes de operar.`);
    }
    if(issues.length){
      banner.innerHTML=`<b>Pré-checagem:</b> ${issues.map(esc).join(" • ")} <button type="button" onclick="openSystemHealth()">Abrir Saúde</button>`;
...
async function showApp(){
  ...
  applyAccessProfile();
  setTimeout(()=>startupPreflight().catch(e=>console.warn("Preflight",e)),300);
  // navegação nunca espera sincronização. Dashboard abre primeiro.
...
async function registerServiceWorkerSafely(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("sw.js?v=4.1.2",{updateViaCache:"none"});
    await reg.update();
    navigator.serviceWorker.addEventListener("message",e=>{
      if(e.data?.type==="CACHE_UPDATED"&&e.data?.version==="4.1.2"){
        localStorage.setItem("okeo_frontend_cache_version","4.1.2")
      }
    })
...
function getCurrentRuntimeErrors(){
  ...
  // Falhas de timeout geradas pela própria Saúde nas versões 4.1.0/4.1.2
  // são diagnóstico legado, não erro operacional.
  return current.filter(e=>!(String(e.scope||"")==="DEEP_HEALTH_CHECK"&&String(e.message||"")==="TIMEOUT_BASE_CENTRAL"))
}
...
async function verifyBackendVersion(expected="4.1.2"){
...
async function repairCentralFromLocal(){
  ...
  try{
    const ver=await verifyBackendVersion("4.1.2");
    if(!ver.ok){
      if(msg)msg.textContent=`Backend incompatível: publicado ${ver.status?.version||"?"}, esperado 4.1.2.`;
      alert(`Publique primeiro o backend 4.1.2. O backend atual responde ${ver.status?.version||"?"}.`);
      return
    }
    ...
    const resumable=progress&&progress.version==="4.1.2";
    if(!resumable){
      ...
      await backup();
      progress={version:"4.1.2",storeIndex:0,rowIndex:0,sent:0,startedAt:new Date().toISOString()};
...
async function runDeepHealthCheck(){
  ...
  if(remote.ok){
    ...
    add("Backend",status.version==="4.1.2",`publicado v${status.version||"?"} • esperado v4.1.2`);
...
async function runSystemSelfTest(){
  ...
  try{
    ...
    add("Backend atual",status.version==="4.1.2",`Versão ${status.version||"?"}`);
...
async function downloadCentralBaseToDevice(){
  ...
  if(currentSession?.role!=="ADMIN")return alert("Apenas Administrador pode executar a carga integral.");
  const pre=await diagnoseDeviceSync();
  if(pre&&(+pre.localCounts.products||0)>100&&(+pre.centralCounts.products||0)<(+pre.localCounts.products||0)){
    return alert("Bloqueado por segurança: este dispositivo possui uma base local maior que a Base Central. Use Corrigir sincronização Local → Central para não perder a base local.")
  }
  if(!confirm("Baixar a Base Central e substituir as cópias locais deste dispositivo? Os dados da Base Central serão preservados."))return;
  if(btn){btn.disabled=true;btn.textContent="Baixando..."}
  try{
    let loaded=0;
    const snapshot={};
    // Primeiro baixa e valida TODOS os stores. Nada local é apagado antes disso.
    for(const store of SYNC_STORES){
      if(status)status.textContent=`Baixando e validando ${DEVICE_SYNC_LABELS[store]||store}...`;
      const r=await apiGet("list",{store});
      if(!Array.isArray(r.rows))throw new Error(`Resposta inválida em ${store}`);
      snapshot[store]=r.rows;
      loaded+=r.rows.length
    }
    if(status)status.textContent="Aplicando Base Central validada...";
    for(const store of SYNC_STORES){
      await clearStore(store);
    ...
    if(compat.snapshotEpoch)localStorage.setItem("okeo_device_snapshot_epoch",compat.snapshotEpoch);
    await selectors();
    await view(document.querySelector(".view:not(.hidden)")?.id||"home");
    if(status)status.textContent=`Carga concluída: ${loaded} registro(s) baixados da Base Central.`;
    await diagnoseDeviceSync()
  }catch(e){
    if(status)status.textContent="Falha na carga integral: "+e.message;
    alert("Carga interrompida antes de concluir. "+e.message)
  }finally{
...
async function hydrateNewDeviceFromCentral(){
  ...
  try{
    const [localProducts,centralCounts]=await Promise.all([countStore("products"),getCentralStoreCounts()]);
    const centralProducts=+centralCounts.products||0;
    // Dispositivo novo/vazio: carrega a Base Central automaticamente em segundo plano.
    if(centralProducts>0&&localProducts<Math.min(20,centralProducts)){
      let loaded=0;
      for(const store of SYNC_STORES){
        const r=await apiGet("list",{store});
      ...
      if(canView(active))await view(active);
      console.info("Dispositivo hidratado pela Base Central",loaded)
    }
